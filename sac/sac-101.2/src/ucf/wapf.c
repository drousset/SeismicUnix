...
{
	char kapfln[133];
	long int ic1, ic2, j, nexday, npkmsc, npksec;
        char *s1;
	...
	if( cmeam.lpfgmt && cmeam.lpfstd ){
		if( (strcmp(kmeam.kpkid,"MWF     ") == 0 || strcmp(kmeam.kpkid
		 ,"WF      ") == 0) || strcmp(kmeam.kpkid,"WAWF    ") == 0
                        ...
                        if (sprintf(kapfln,"%16s%8s%7.2f%7.2f%4s%5ld%3ld%3ld%3ld%6.2f %10.4g %1s %3s%c"
			...
			for( j = 2; j <= 5; j++ ){
                        ...
                        if(sprintf(kapfln,"%16s%8s%7.2f%7.2f%4s%5ld%3ld%3ld%3ld%6.2f %10.4g %1s %3s%c%6.3f%10.4g\n",
			 kevnm, kstnm, *cmpaz, *cmpinc, kmeam.kpkid, cmeam.npkyr, 
			 cmeam.npkjdy, cmeam.npkhr, cmeam.npkmn, cmeam.pksecs, 
		...
		if( (strcmp(kmeam.kpkid,"MWF     ") == 0 || strcmp(kmeam.kpkid
		 ,"WF      ") == 0) || strcmp(kmeam.kpkid,"WAWF    ") == 0
                        ...
                        s1[ic2-ic1+1] = '\0';
                        if(sprintf(kapfln,"%32s      %4s%5ld%3ld%3ld%3ld%6.2f %10.4g %1s %3s%c",
			...
			for( j = 2; j <= 5; j++ ){
                        ...
                        strncpy((s1=malloc(ic2-ic1+2)),kmdfm.kdfl+ic1-1,ic2-ic1+1);
                        s1[ic2-ic1+1] = '\0';
                        if(sprintf(kapfln,"%32s      %4s%5ld%3ld%3ld%3ld%6.2f %10.4g %1s %3s%c%6.3f%10.4g\n",
			 s1, kmeam.kpkid, cmeam.npkyr, cmeam.npkjdy, cmeam.npkhr, cmeam.npkmn, 
			 cmeam.pksecs, cmeam.pkampl, kmeam.kpksrc, kmeam.kpkrid
			 , 'C', Dtwf[4], Awf[4] ) < 0) {
                          free(s1);
