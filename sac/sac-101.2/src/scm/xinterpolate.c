...
long int *nerr;
{
	long int j, j_, jdfl, jdfl_, ndecr,
...
L_1000:
	...
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
		...
		if( cmscm.lbreq ){
			xstart = cmscm.breq;
			if( xstart < *b ){
				nincr = (long)( (*b - xstart)/cmscm.dtnew ) + 1;
		...
		if( cmscm.lnreq ){
			...
			if( xstop > *e ){
				ndecr = (long)( (xstop - *e)/cmscm.dtnew ) + 1;
				newlen = newlen - ndecr;
			...
			newlen = (long)( *delta*((float)( *npts )/cmscm.dtnew) );
			xstop = xstart + (float)( newlen - 1 )*cmscm.dtnew;
			if( xstop > *e ){
				newlen = newlen - 1;
			...
			newlen = (long)( (*e - xstart)/cmscm.dtnew );
			xstop = *e;
			}
