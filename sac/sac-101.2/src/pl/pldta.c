...
float xarray[], yarray[];
long int number, incx, incy, *nerr;
{
	char kinc[9];
	long nXdata = 0 , nYdata = 0 ; /* number of points plotted so far, maf 980116 */
	long lclip, lfdon, lnewdp, lqdp, ltdon;
	long int inc, insym, j, j3, 
	...
	*nerr = 0;
	...
	if( cmgem.lxgen ){
		...
		numf = 1 + (long)( (cmgem.ximnu - xtest)/cmgem.xdelta );
	    xtest = cmgem.xfirst + (float)( number - 1 )*cmgem.xdelta - 
	     VSMALL;
	    if( xtest > cmgem.ximxu )
		numl = number - (long)( (xtest - cmgem.ximxu)/cmgem.xdelta );
	}
	else if( cmgem.lygen ){
	    ytest = cmgem.yfirst + VSMALL;
	    if( ytest < cmgem.yimnu )
		numf = 1 + (long)( (cmgem.yimnu - ytest)/cmgem.ydelta );
	    ytest = cmgem.yfirst + (float)( number - 1 )*cmgem.ydelta - 
	     VSMALL;
	    if( ytest > cmgem.yimxu )
		numl = number - (long)( (ytest - cmgem.yimxu)/cmgem.ydelta );
	}
