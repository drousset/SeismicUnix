...
{
	long int jdx, jx, jy, num1, num2, num2m1;
	...
	*nerr = 0;
	...
	if( cmgem.lylim ){
		...
		if( cmgem.lxlim ){
		    if( cmgem.lxgen ){
			num1 = (long)( (cmgem.ximnu - cmgem.xfirst)/cmgem.xdelta ) + 1;
			if( num1 < 1 )
			    num1 = 1;
			num2 = (long)( (cmgem.ximxu - cmgem.xfirst)/cmgem.xdelta ) + 1;
			if( num2 > number )
			    num2 = number;
	...
	if( cmgem.ixint == cmgem.ilin ){
		...
		if( cmgem.lxfull ){
		    ...
		    if( tmp < 0. )
			tmp = tmp - 0.99999;
		    cmgem.ximnz = (float)( (long)( tmp ) );
		    tmp = cmgem.ximxz;
		    if( tmp > 0. )
			tmp = tmp + 0.99999;
		    cmgem.ximxz = (float)( (long)( tmp ) );
		} /* end if( cmgem.lxfull ) */
	    } /* end if( !cmgem.lxfull ) */
	...
	if( cmgem.iyint == cmgem.ilin ){
		...
		if( cmgem.lyfull ){
		    ...
		    if( tmp < 0. )
			tmp = tmp - 0.99999;
		    cmgem.yimnz = (float)( (long)( tmp ) );
		    tmp = cmgem.yimxz;
		    if( tmp > 0. )
			tmp = tmp + 0.99999;
		    cmgem.yimxz = (float)( (long)( tmp ) );
		} /* end if( cmgem.lyfull ) */
	    } /* end if( !cmgem.lylim ) */
