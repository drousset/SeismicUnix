...
long int *nerr;
{
	long int idx, idd, ideg, ifrac, ihh, ijday, imm, imsec, 
	...
	*nerr = 0;
	...
	nlcmem = Ndxhdr[idfl];
	copy( (long*)cmmem.sacmem[nlcmem], (long*)&Fhdr[1], MCMHDR );
	zgetc( (long int *)cmmem.sacmem[nlcmem]+MCMHDR, kmhdr.khdr[0], (MCPW+1)* MKHDR );
	...
	if( *delta != cmhdr.fundef )
		*isdelt = (long)( (1.0/ *delta)*100.0 + .5 );
	*isnpts = *npts;
	if( *stel != cmhdr.fundef )
		*issel = (long)( *stel*100.0 + .5 );
	if( *stdp != cmhdr.fundef )
		*issdep = (long)( *stdp*100.0 + .5 );
	...
	*isdate = idd + (*nzyear*100 + imm)*100;
	...
	nlcmem = Nxsdd[idfl];
	if( nlcmem != 0 ){
                Isacmem = (long *)cmmem.sacmem[nlcmem];
		*isclas = *(Isacmem++);
		*isfrmt = *(Isacmem++);
		*iscalg = *(Isacmem++);
		nlcmem = nlcmem + 3;
