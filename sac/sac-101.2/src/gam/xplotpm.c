...
long int *nerr;
{
	...
	long lany, lframesave, log, lrxlim, lwait, lprint = FALSE , ltry = FALSE ;
	long int ixplot, iyplot, jdfl, junk, nc, ncret, ndxx, 
	...
	*nerr = 0;
	...
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl += 2 ){
		...
		start = fmax( tmin, *b );
		stop = fmin( tmax, *e );
		iyplot = (long)( (start - *begin)/ *delta );
		nplot = (long)( (stop - start)/ *delta );
