...
#include "../../inc/nvars.h"
#include "../../inc/mem.h"
long /*FUNCTION*/ encodeventry(index, name, name_s, value, value_s)
long int *index;
char *name;   int name_s;
char *value;   int value_s;
{
	char dtemp[25], temp[17];
	long done, encodeventry_v;
	long int _l0, jvalue, nc, ncv1, ncv2, ncvmax, offset;
	float singletemp;
	double doubletemp;
	...
	decodevdesc( (long int *)cmmem.sacmem[cmvars.varsnode1]+offset, &cmvars.deleteflag, &cmvars.readonlyflag, 
	...
	if( cmvars.valuetype == VALUENIL ){
		...
		*index = cmvars.descindex + cmvars.desclength;
		...
		zgetc( (long int *)(cmmem.sacmem[cmvars.varsnode1]+offset), name, 4*cmvars.namelength );
		*index = *index + cmvars.namelength;
		...
		if( cmvars.valuetype == VALUELIST ){
			...
			fstrncpy( value, value_s-1, " ", 1 );
                        offset = *index - cmvars.varsnode1;
			zgetc( (long int *)(cmmem.sacmem[cmvars.varsnode1]+offset), value, 4*cmvars.valuelength );
			}
		else if( cmvars.valuetype == VALUEINTEGER ){
L_1000:
			if( !done ){
                                offset = *index - cmvars.varsnode1;
				cnvita( (long)cmmem.sacmem[cmvars.varsnode1]+offset, temp,17 );
				*index = *index + 1;
				ljust( temp,17 );
				nc = indexb( temp,17 );
				ncv2 = ncv1 + nc;
