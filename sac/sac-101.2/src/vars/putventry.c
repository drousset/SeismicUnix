...
long int *type, length, *index, *nerr;
{
	long roflag;
	long int _l0, copyfrom, copyto, difflength, nchars, newlength, 
...
L_1000:
	*nerr = 0;
	...
	if( existsv( vars,vars_s, name,name_s ) ){
      ...
      malloc_verify();
#endif
                        encodevdesc( (long int *)Sacmem, &cmvars.deleteflag, &cmvars.readonlyflag, 
      ...
      malloc_verify();
			...
			zputc( kmvars.valuename,MAXCVNAME+1, (long int *)Sacmem, 4*cmvars.namelength );
