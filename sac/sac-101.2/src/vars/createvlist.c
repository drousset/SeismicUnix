...
long int length, *node, *nerr;
{
	char ktemp[MAXCVNAME+1];
	long int _l0, icchild1, icchild2, icparent2, index, nchars, niloffset, 
	 type;
	...
	*nerr = 0;
	...
	encodevdesc( (long int *)cmmem.sacmem[index], &cmvars.deleteflag, &cmvars.readonlyflag, 
	...
	zputc( kmvars.valuename,
	       MAXCVNAME+1, 
	       (long int *)(cmmem.sacmem[Varsindex[*node]]+cmvars.desclength),
	       (long int)(4*cmvars.namelength) );
