...
{
	char lastword[5] = "    " ;
	long int _l0, index, length, numwhole, numwords, type;
	void zputc();
  ...
  malloc_verify();
	...
	if( 4*numwords == length ){
	  zputc( value,value_s, (long int *)Sacmem, 4*numwords );
		}
	else{
		numwhole = numwords - 1;
		if( numwhole >= 1 )
		  zputc( value,value_s, (long int *)Sacmem, 4*numwhole );
		fstrncpy( lastword, 4, value+numwhole*4,length - (4*numwhole + 
		 1) + 1);
		zputc( lastword,5, (long int *)(Sacmem+numwords-1), 4 );
		}
