...
#include "../../inc/nvars.h"
#include "../../inc/mem.h"
long /*FUNCTION*/ existsv(vars, vars_s, name, name_s)
char *vars;   int vars_s;
char *name;   int name_s;
{
	long existsv_v;
	long int _l0, ncname, node, nvname;
	...
	if( existsvlist( vars,vars_s, "BOTH", &node ) ){
		...
		decodevdesc( (long int *)cmmem.sacmem[cmvars.descindex], &cmvars.deleteflag, 
...
L_1000:
	...
	decodevdesc( (long int *)Sacmem, &cmvars.deleteflag, &cmvars.readonlyflag, 
	...
	zgetc((long int *) Sacmem, kmvars.valuename, 4*cmvars.namelength );
