...
long int *node, *nerr;
{
	...
	long lopen;
	long int index, itemp[10], ncerr, nderr, nilindex, nlocdisk, 
	 numwords, nun;
	void decodevdesc(), zgetc(), zrabs();
        char *s1;


	long *const Itemp = &itemp[0] - 1;
	...
	*node = 0;
	...
	nlocdisk = 0;
	numwords = 10;
	zrabs( (int *)&nun, (char *)itemp, numwords, (int *)&nlocdisk, (int *)nerr );
	if( *nerr != 0 )
		goto L_8888;
	zgetc( (long int *)kmvars.varsidcode, varsid, 4 );
	...
	zrabs( (int *)&nun, (char *)(cmmem.sacmem[index]), numwords, (int *)&nlocdisk, (int *)nerr );
	if( *nerr != 0 ){
                apcmsg2(fullvars+1,fullvars_s - 2);
		goto L_8888;
	}
	...
	decodevdesc( (long int *)cmmem.sacmem[index], &cmvars.deleteflag, &cmvars.readonlyflag, 
	 &cmvars.indirectflag, &cmvars.sharedflag, &cmvars.reservedflag, 
	 &cmvars.applflag1, &cmvars.applflag2, &cmvars.valuetype, &cmvars.desclength, 
	 &cmvars.namelength, &cmvars.valuelength );
