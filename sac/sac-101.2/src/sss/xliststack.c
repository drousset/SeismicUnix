...
long int *nerr;
{
	...
	long int ic1, ic2, j, jdfl, jdfl_, jvm, jvm_;
	...
	for( jvm = 1; jvm <= MVM; jvm++ ){
		...
		if( Lvm[jvm] ){
			if( Ivm[jvm] == cmsss.inmo ){
                                sprintf(kline," Stack Velocity Model %1ld %8s", jvm, knmo );
				}
			else if( Ivm[jvm] == cmsss.irefr ){
                                sprintf(kline," Stack Velocity Model %1ld %8s", jvm, krefr );
				}
			aplmsg( kline,MCMSG+1 );
			if( Lcvapp[jvm] ){
			...
			aplmsg( kline,MCMSG+1 );
                        ...
                        sprintf(kline," Stack Velocity Model %1ld %8s",jvm,koff);
			aplmsg( kline,MCMSG+1 );
			}
