...
struct t_brkc {
	...
	long int nin, nph0, int0[2], ki, msrc[2], isrc[2], nseg, nbrn, 
...
#define PCNTL(I_,J_)	(pcntl+(I_)*(pcntl_s)+(J_))
	...
	static long all, fnd;
	static long int i, i_, j, j1, j1_, j2, j2_, j_, k, k_, kseg, l, 
	 l_, no, nsgpt[JBRN];
	...
	static char cmdlst[LCMD][9]={"P       ","PKiKP   ","PcP     ",
	 ...
	 "pS      "};
	static long ncmpt[NCMD][2]={1,2,1,7,1,13,13,16};
        char *s1;


	long *const Iidx = &brkc.iidx[0] - 1;
	long *const Int0 = &brkc.int0[0] - 1;
	long *const Isrc = &brkc.isrc[0] - 1;
	long *const Jidx = &brkc.jidx[0] - 1;
	long *const Kk = &brkc.kk[0] - 1;
	long *const Km = &brkc.km[0] - 1;
	long *const Ku = &brkc.ku[0] - 1;
	long *const Msrc = &brkc.msrc[0] - 1;
	long *const Nsgpt = &nsgpt[0] - 1;
	double *const Pk = &brkc.pk[0] - 1;
	long *const Prflg = &prflg[0] - 1;
	long *const Prnt = &prtflc.prnt[0] - 1;
	long *const Segmsk = &prtflc.segmsk[0] - 1;
...
L_24:
	...
	for( i = 1; i <= brkc.nseg; i++ ){
		...
		fprintf( stdout, "Brnset:  Segment pointer (%ld ) missing?\n", 
		 i );
		goto L_16;
L_18:
...
long int nph;
{
	static long noend, noext;
	long int _i, _r;
	static long int i, i1, i2, i_, iph, is, j, j_, k, k1, k2, k_, 
...
float usrc[];
{
	static long dop, dos;
	long int _i, _r;
	static long int i, i_, ind, int_, j, j_, k, nph;
	static float rdep;
	static int _aini = 1;
...
*/
	if( _aini ){ /* Do 1 TIME INITIALIZATIONS! */
		for(_i=0L; _i < sizeof(prtflc.segmsk)/sizeof(long); _i++)
			prtflc.segmsk[_i] = TRUE;
		for(_i=0L; _i < sizeof(prtflc.prnt)/sizeof(long); _i++)
...
#define PHNM(I_,J_)	(phnm+(I_)*(phnm_s)+(J_))
	static char msg[68];
	static long int i, i_, ie, ij, ij_, in, is, j, jj, jj_, le, ln, 
	 nph;
		...
		for( i = is; i <= ie; i++ ){
			...
			*n = *n + 1;
			...
			fstrncpy( PHNM(*n - 1,0), phnm_s-1, pcdc.phcd[jb - 1],
                        ...
                        if ( (in = (long) (strstr(PHNM(*n - 1,0), "ab" ))) != 0)
                                              in -= (long)PHNM(*n - 1,0) +1;
...
/*		ln = istrstr( pcdc.phcd[jb - 1], " " ) - 1; */
                ...
                if ( (ln = (long)strstr(pcdc.phcd[jb - 1]," ")) != 0)
                                  ln -= (long)pcdc.phcd[jb - 1] +1;
...
L_10:
	...
	return;
L_13:
        fprintf(stdout,"More than %3ld arrivals found.\n",max_);
...
{
	static long int i, i_, ie, is, j, j_, n, n1;
...
long /*FUNCTION*/ iupcor(phnm, dtdd, xcor, tcor)
char *phnm;
double dtdd;
float *xcor, *tcor;
{
	static long int i, i_, ie, is, iupcor_v, j, jb, jp_, js_;
...
long int int_, *len;
{
	static long int i, i_, ie, is, j, j_, k, m, n;
...
L_15:
	...
	ie = ie + 1;
L_16:
	n = max( (long)( fabs( xs - x0 )/xmin + .8e0 ), 1 );
	dx = (xs - x0)/n;
	dx2 = fabs( .5e0*dx );
	sgn = sign( 1e0, dx );
	rnd = 0e0;
...
long int iptr[];
{
	static long int i, i_, ib, ij, il[10], it, iu[10], j, k, kk, l, 
	 m;
	static float r, tmpkey;

	long *const Il = &il[0] - 1;
	long *const Iptr = &iptr[0] - 1;
	long *const Iu = &iu[0] - 1;
...
long int jb, int_;
{
	...
	static long makgrd, newgrd;
	static long int i, i1, i2, i_, ios, is, j, j_, k, mncnt, mxcnt, 
	 nn;
...
char *modnam;   int modnam_s;
{
	...
	short int _i0;
	long int i, i_, idx, ind, ireterror, j, j_, k, l, len2, nasgr, 
...
/*	fscanunf( UFP(brkc.nin), "%& ", sizeof(brkc.coef),brkc.coef );  */
	...
	fread(&size1, sizeof(int), 1, file);
		fread(&nasgr    , sizeof(long int), 1      , file);
		fread(&nl       , sizeof(long int), 1      , file);
		fread(&len2     , sizeof(long int), 1      , file);
		fread(&tabc.xn  , sizeof(float   ), 1      , file);
		fread(&tabc.pn  , sizeof(float   ), 1      , file);
		fread(&tabc.tn  , sizeof(float   ), 1      , file);
		fread( umdc.mt  , sizeof(long int), 2      , file);
		fread(&brkc.nseg, sizeof(long int), 1      , file);
		fread(&brkc.nbrn, sizeof(long int), 1      , file);
		fread( brkc.ku  , sizeof(long int), 2      , file);
		fread( brkc.km  , sizeof(long int), 2      , file);
		fread( brkc.fcs , sizeof(float   ), 3*JSEG , file);
		fread( brkc.nafl, sizeof(long int), 3*JSEG , file);
		fread( brkc.indx, sizeof(long int), 2*JSEG , file);
		fread( brkc.kndx, sizeof(long int), 2*JSEG , file);
	...
	fread(&size1, sizeof(int), 1, file);
		...
		fread( umdc.ndex, sizeof(long int), 2*JSRC , file);
	fread(&size2, sizeof(int), 1, file);
	if (size1 != size2) {fprintf(stderr, "bad sizes\n"); exit(-1);}
	fread(&size1, sizeof(int), 1, file);
	...
	fread(&size1, sizeof(int), 1, file);
		...
		*/
		...
		fread( tabc.jndx, sizeof(long int), 2*JBRN , file);
	fread(&size2, sizeof(int), 1, file);
	if (size1 != size2) {fprintf(stderr, "bad sizes\n"); exit(-1);}
	fread(&size1, sizeof(int), 1, file);
...
double pt[], coef[][5];
{
	static long int i, i_, is, j, j_, k, k_, l, m, n2;
...
#define PHNM(I_,J_)	(phnm+(I_)*(phnm_s)+(J_))
	...
	static long int i, i_, iptr[60], j, j_, k;
...
{
	double zs = _umod.zs;
	long int *isrc = _umod.isrc;
	long int nph = _umod.nph;
	double uend = _umod.uend;
	long int js = _umod.js;
	static char msg[32];
	static long int i, i_, j, m1;
	static float dep;
/*	void abort();  */
	static double dtol = 1e-6;

	long *const Isrc = &isrc[0] - 1;
	long *const Mt = &umdc.mt[0] - 1;
	double *const Pt = &tabc.pt[0] - 1;
	double *const Us = &tabc.us[0] - 1;
...
long int *nerr;
{
	...
	long all, fnd, prnt, segmsk;
	long int ncmpt[NCMD][2], nsgpt[JBRN];
