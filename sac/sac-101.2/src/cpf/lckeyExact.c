...
#include "mach.h"
#include "com.h"
long /*FUNCTION*/ lckeyExact(kkey, kkey_s)
char *kkey;   int kkey_s;
{
	char kcheck[137], ktoken[137];
	long lckeyExact_v, lnoabb;
	long int idx, j, jc, jcopy, jicom, jsym, ncabb, ncheck, nckey, 
	 ncsym;
        char *strtemp;
	...
	if( kmcom.kcom[cmcom.jcom - 1][0] == '\'' ){
	    jsym = cmcom.jcom + 2;
	    ncsym = (long)( Flnum[cmcom.jcom + 1] + 0.1 );

	    /* -- Check ktoken size, truncate and issue warning if token is too long. */
	    if( ncsym > MCHECK ){
		ncsym = MCHECK;
		setmsg( "WARNING", 921 );
		wrtmsg( MUNOUT );
