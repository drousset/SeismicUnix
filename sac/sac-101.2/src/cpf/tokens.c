...
char *kstrg;
long int ncstrg, mtok;
char *ktok;   int ktok_s;
long int *jtok, *jcstrg;
{
#define KTOK(I_,J_)	(ktok+(I_)*(ktok_s)+(J_))
	...
	long lmsg, lnwtok;
	byte kchar, klchar;
	long int check, imsgcc, imsgtp, itoksv, jnwtok, mtokm1;
	...
	*jcstrg = *jcstrg + 1;
	if( *jcstrg <= ncstrg ){
		...
		if( imsgtp == 1 && memcmp(&kchar,kcurdl,1) == 0 ){
		  ...
		  check = imsgtp == 2 && nequal ( &kchar, (char *) &kmtpf.ktokdl ,
						    1 , cmtpf.ntokdl ) > 0 ;
		    if( check ){
			lmsg = FALSE;
			if( jnwtok >= 1 ){
		...
		if( (klchar != ' ' && jnwtok >= 1) || lnwtok ){
	      ...
	      check = nequal( &kchar, (char *)&kmtpf.ktokdl,1, cmtpf.ntokdl ) > 0;
		if( check ){
		    if( jnwtok >= 1 ){
			*jtok = *jtok + 1;
			fstrncpy ( KTOK ( *jtok - 1 , 0 ) , ktok_s - 1 ,
		  ...
		  check = nequal( &kchar, (char *)&kmtpf.kmsgdl,1, cmtpf.nmsgdl ) > 0;
		    if( check ){
			if( jnwtok >= 1 ){
			    *jtok = *jtok + 1;
			    fstrncpy ( KTOK ( *jtok - 1 , 0 ) , ktok_s - 1 ,
