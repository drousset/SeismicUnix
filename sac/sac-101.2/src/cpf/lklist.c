...
#include "mach.h"
#include "com.h"
long /*FUNCTION*/ lklist(kkey, kkey_s, klist, klist_s, nlist, index)
char *kkey;   int kkey_s;
char *klist;   int klist_s;
long int nlist, *index;
{
#define KLIST(I_,J_)	(klist+(I_)*(klist_s)+(J_))
	char ktoken[9];
	long lklist_v;
	long int imatch[MMATCH], j, j_, jicom, nchar, nerr, nmatch;

	long *const Imatch = &imatch[0] - 1;
...
L_2000:
		...
		nchar = MCPW;
		jicom = ((long)( Flnum[cmcom.jcom + 1] + 0.1 ) - 1)/MCPW + 3;
	    }
	    else{
		modcase( TRUE, (char*)kmcom.kcom[cmcom.jcom - 1],MCPW, ktoken );
