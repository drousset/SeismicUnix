...

{
  ...
  if( header_in != NULL ){ /* make a copy of the input header */
    ...
    setnhdr(new_header, "nvhdr",  (long)MVHDRC, &nerr);  /* version       */
    setlhdr(new_header, "leven",  (long)TRUE,   &nerr);  /* evenly spaced */
    setlhdr(new_header, "lovrok", (long)TRUE,   &nerr);  /* overwrite ok  */
    setlhdr(new_header, "lcalda", (long)TRUE,   &nerr);  /* dist, az, baz */
                                                         /* and gcarc are */
                                                         /* calc from station*/
                                                         /* & event coords. */
    /* ninf and nhst were changed to norid and nevid.  maf 961031 */
/*    setnhdr(new_header, "ninf"  , (long)0,      &nerr);
    setnhdr(new_header, "nhst"  , (long)0,      &nerr);		*/
    setehdr(new_header, "iftype", (long)ITIME,  &nerr);
  }
