 ...
 *
 * */
void /*FUNCTION*/ C_buroots(p, rtype, rtype_s, dcvalue, nsects, iord)
complexf p[];
char *rtype;   int rtype_s;
float *dcvalue;
long int *nsects, iord;
{
#define RTYPE(I_,J_)	(rtype+(I_)*(rtype_s)+(J_))
	...
	*nsects = 0;
	if( 2*half < iord ){
		P[1] = flttocmplx( -1., 0. );
		C_fstrncpy( RTYPE(0,0) , rtype_s - 1 , "SP", 2 );
		*nsects = 1;
		}
	...
	for( k = 1; k <= half; k++ ){
		...
		*nsects = *nsects + 1;
		...
		C_fstrncpy( RTYPE(*nsects - 1,0) , rtype_s - 1 , "CP", 2 );
		}
