...
void /*FUNCTION*/ pickwindows(envelopes,nbands,C_begin,nerr)
     ...
     long int *nerr;
{
	...
	long lany, lempty, lhlwrt[MDFL], lppkab, lrdttm, ltitls, 
	 lwfok, lxlims, lzdttm[MDFL];
	byte kchar;
	long lfirst, lxlimj, lylimj,num, num1, num2, num2m1, jx, jy, ic1, ic2, nc;
	float atrwid, fjunk, slen, slenm, ximnj, ximxj, xjunk, 
	 xlinl1, xlinl2, xsymlc, yatrlc, yimnj, yimxj;
	long int iwf[5], jdx, jdfl, jdfl1, jdfl2, ndfl,
	...
	static long lint = FALSE;
	static long lnewxw = FALSE;
	static byte kdir = 'U';
	static byte ktype = 'I';
	static byte kqual = '0';
	static long lhltrm = FALSE;
	static long lhlhyp = FALSE;
	static long nsavelocs = 0;
        char *cattemp, *cattemp1;


	long *const Iwf = &iwf[0] - 1;
	long *const Lhlwrt = &lhlwrt[0] - 1;
	long *const Lzdttm = &lzdttm[0] - 1;
	float *const Toff = &toff[0] - 1;
	float *const Yimnzs = &yimnzs[0] - 1;
	float *const Yimxzs = &yimxzs[0] - 1;

        extern long int bellON;
...
L_2000:
	...
	*ennd = *begin + (float)( *npts - 1 )**delta;
	...
	for( jdfl = 1; jdfl <= ndfl; jdfl++ ){
		...
		if( lxlims && !lylimj ){
		    if( *leven ){
			num1 = (long)( (ximnj - *begin)/ *delta ) + 1;
			if( num1 < 1 )
			    num1 = 1;
			num2 = (long)( (ximxj - *begin)/ *delta ) + 1;
			if( num2 > nlen )
			    num2 = nlen;
			if( num1 <= nlen && num2 >= 1 ){
			    num2m1 = num2 - num1 + 1;
	...
	for( jdfl = 1; jdfl <= ndfl; jdfl++ ){
		...
		if( cmgam.ifidtp == 4 ){
			...
			text( ktemp , ( nc = strlen ( ktemp ) ) + 1 , nc );
		    }
		}
