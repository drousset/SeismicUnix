...
long int number, status, numlocs, firstloc;
{
	...
	if( number <= cmcontouring.numsegments ){
                Isacmem = (long *)cmmem.sacmem[cmcontouring.indexseglabelst];
		*(Isacmem + number - 1) = status;

                Isacmem = (long *)cmmem.sacmem[cmcontouring.indexseglabelnu];
		*(Isacmem + number - 1) = numlocs;

                Isacmem = (long *)cmmem.sacmem[cmcontouring.indexseglabelfi];
		*(Isacmem + number - 1) = firstloc;
		}
	else{
		fprintf( stdout, "Illegal labeled segment number: %ld \n", 
		 number );
		}
