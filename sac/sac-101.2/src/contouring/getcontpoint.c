...
long int *link, *action;
{
	...
	if( number <= cmcontouring.numpoints ){
		...
		Point[2] = *(cmmem.sacmem[cmcontouring.indexpoints] + 2*(number - 1) + 1);
                Isacmem = (long*)cmmem.sacmem[cmcontouring.indexaction];
		*action = *(Isacmem + number - 1);
                Isacmem = (long*)cmmem.sacmem[cmcontouring.indexlinks];
		*link = *(Isacmem + number - 1);
	}
	else{
		fprintf( stdout, "Illegal point number:%ld getcontpoint \n", number );
		exit(0);
	}
