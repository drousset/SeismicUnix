...
float zdata[];
long int nxsize, nysize, *nerr;
{
	char _c0[2], kformat[9], khorz[9], kvert[9];
	long lfast;
	long int ilabel, j, j_, jlevel, jlevel_, jzregion, nc;
	...
	*nerr = 0;
	...
	if( strcmp(kmcontouring.klevelmode,"LIST    ") == 0 ){
		...
		if( cmcontouring.zlevelincrement > 0.0 ){
			...
			zminimum = zincrement*(float)( (long)( zminimum/zincrement ) );
			zmaximum = zincrement*(float)( (long)( zmaximum/zincrement ) + 
			 1 );
			cmcontouring.nzlevels = min( (long)( (zmaximum - zminimum)/
