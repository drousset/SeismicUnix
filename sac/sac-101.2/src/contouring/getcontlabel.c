...
long int *jtext;
{
	...
	if( number <= cmcontouring.numlabels ){
                Isacmem = (long*)cmmem.sacmem[cmcontouring.indexlabelpoint];
		*jpoint = *(Isacmem + number - 1);
                Isacmem = (long*)cmmem.sacmem[cmcontouring.indexlabeltype];
		*jtype = *(Isacmem + number - 1);
		*angle = *(cmmem.sacmem[cmcontouring.indexlabelangle] + number - 1);
                Isacmem = (long*)cmmem.sacmem[cmcontouring.indexlabeltext];
		*jtext = *(Isacmem + number - 1);
	}
	else{
		fprintf( stdout, "Illegal label number:%ld \n", number );
		exit(0);
	}
