...
long int index, ilabel;
double width;
float *angle;
{
	long lchange;
	long int jaction[MPOINTS], jlink[MPOINTS], jloc, jlocm1, jlocm2, 
	 jlocp1, jlocp2, jpoint[MPOINTS], jrlink[MPOINTS], jtemp;
	float anglem12, anglep12, anglepm2, correction, distance, fpoint[MPOINTS][2];

	long *const Jaction = &jaction[0] - 1;
	long *const Jlink = &jlink[0] - 1;
	long *const Jpoint = &jpoint[0] - 1;
	long *const Jrlink = &jrlink[0] - 1;
