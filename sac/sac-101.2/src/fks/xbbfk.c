...
long int *nerr;
{
	...
	long lformer, llocal, lssq_specified , lprint = FALSE , ltry = FALSE ;
	long int idx, iasiz[2], idummy, isacm, issq, ix, iy,
	...
	long *const Iasiz = &iasiz[0] - 1;
	long *const Nsamps = &nsamps[0] - 1;
	long *const Ptr = &ptr[0] - 1;
	...
	if( nch < 3 || nch > MXLENS ){
	    ...
	    /*            setmsg( "ERROR", *nerr ) ;
            ...
            outmsg() ; */
	    fprintf( MUNOUT, "Number of files must be between 3 and MXLENS=%ld \n", MXLENS );
	    goto L_8888 ;
	}
	...
	while ( lcmore( nerr ) ){
		...
		if( (((cmfks.iazs%2) != 0) || ((cmfks.iwvs%2) != 0)) || 
		 (cmfks.iazs*cmfks.iwvs > MXLENB) ){
		    fprintf( MUNOUT, "ERR: For SIZE, you must give 2 even nums whose product is < %ld \n", 
		     MXLENB );
		    goto L_8888 ;
		}
	    }
		...
		if( ((cmfks.idimsq%2) != 0) || cmfks.idimsq*cmfks.idimsq > 
		 MXLENB ){
		    fprintf( MUNOUT, "ERR: For WRITE, num of samples squared must be < %ld \n", 
		     MXLENB );
		    goto L_8888 ;
		}
	    }
	...
	ns = nssav;
	fprintf( MUNOUT, "NUM TIME-SERIES FILES FROM THE START OF DFL:%ld \n", ns );
	if( ns < 3 || ns > MXLENS ){
	    fprintf( MUNOUT, "ERR: THIS NUM MUST BE FROM 3 TO %ld \n", MXLENS );
	    goto L_8888 ;
	}
...
float **data;
long int ptrs[], nch, nsamples;
long ltofilter;
complexf scm[];
long int *nerr;
{
	long int decrate, idx, index1, index2, iptr, jdx, jptr, jptr0, 
...
long maxflag;
{
	long int half, idx, ic, id, ifks, jdx, mdx, ndx, offset;
...
float fks[];
{
	long int halfns, idx, ic, id, ifks, ifksr, jdx, mdx, maxfks, ndx;
...
#define S(I_,J_)	(*(s+(I_)*(nm)+(J_)))
	...
	if( memcmp(hires,"MLM",3) == 0 || memcmp(hires,"mlm",3) == 0 ){
		fprintf( MUNOUT, "      mlm %ld \n", m );
		fprintf( MUNOUT, "   BBFK computed with eigen terms [vectors & values]\n" );
		fprintf( MUNOUT, "   that have the HIGHEST%ld  eigenvalues.\n", m );
		fputc( '\n', MUNOUT );
		...
		for( kdx = 1; kdx <= n; kdx++ ){
			if( kdx >= n - m + 1 ){
                                fprintf(MUNOUT," %c        %2ld           %2ld         %21.10f\n", '*', 
				 kdx, n - kdx + 1, W[kdx] );
				}
			else{
                                fprintf(MUNOUT," %c        %2ld           %2ld         %21.10f\n", ' ', 
		...
		*nerr = 0;
		...
		fprintf( MUNOUT, "      mus %ld \n", m );
		fprintf( MUNOUT, "   BBFK computed with eigenvectors associated\n" );
		fprintf( MUNOUT, "   with the LOWEST%ld  eigenvalues.\n", m );
		fputc( '\n', MUNOUT );
		fprintf( MUNOUT, "(*)used  ascend(MUS)  descend(MLM)          eigenvalue\n" );
