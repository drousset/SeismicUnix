...
long int *nerr;
{
	...
	long elevc;
	long int iadv, idummy, inptr, jdx, jdfl, jdfl_, joptr, jout, 
	...
	long  lLocalRef ;	/* these three added for reference option. */
	int   nLocalRef ;	/* maf 970207 */
	float rLocalRef[ 3 ] ;

	long  lfillz = 0 ;	/* set to 1 if npts or begins differ. maf 970211 */
	...
	for( jdfl = 1; jdfl <= number; jdfl++ ){
	    ...
	    iadv = (long) ( advance >= 0 ? advance / *delta + 0.5 :
					   advance / *delta - 0.5 ) ;
