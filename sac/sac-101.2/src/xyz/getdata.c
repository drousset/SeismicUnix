...
#include "../../inc/mem.h"
long /*FUNCTION*/ getdata(nfiles, delta, windwsize, windwovrl, windwfunc, 
	 buffersize, filelength, bufindex, signals)
long int nfiles;
double delta;
long int windwsize, windwovrl;
char *windwfunc;
long int buffersize, filelength[], bufindex;
float signals[];
{
	long int err, error, getdata_v, i, i_, index, j, j_, lread;
	float dum1, dum2;
...
L_1:
				...
				if( (!(filesinfo.bufferfull) && (!(filesinfo.nodata)
					...
					if( error == 0 ){
						...
						getfil( filesinfo.ifile, TRUE, (long*)&dum1, 
						 &index, (long*)&dum2, &err );
						if( err != 0 ){
							error = 1;
							}
