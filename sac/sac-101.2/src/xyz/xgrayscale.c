...
long int *nerr;
{
	...
	long lvideo, lzoom;
	long int graylength, graywidth, imagelength, imagewidth, jfile, 
	 jfile_, ndxz, nfiles, nlen, notused, nxsize, nysize, nzsize, 
	 un, xmax, xmin, ymax, ymin;
	static long maximagesize = 2050;
	static byte videotype = 'n';
	static float scale = 1.0;
	static long zoom = 0;
	static long xcrop = FALSE;
	static long ycrop = FALSE;
	static long xcropmin = 0;
	static long xcropmax = 0;
	static long ycropmin = 0;
	static long ycropmax = 0;
...
L_1000:
	...
	for( jfile = 1; jfile <= nfiles; jfile++ ){
		...
		/* -- Format argument list for "display." */
                sprintf(arg1,"%10ld%10ld%10.2f %c%10ld%10ld%10ld%10ld%10ld%10ld%10ld%10ld%s\n",
