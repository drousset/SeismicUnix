...
long /*FUNCTION*/ calcfftsize(delta, sliceint, window, iorfft, lfft, 
	 nptswndw, buffersize, windowovrl)
double delta, sliceint, window;
long int *iorfft, *lfft, *nptswndw, *buffersize, *windowovrl;
{
	long int calcfftsize_v, n;
...
L_3:
	...
	*buffersize = n**nptswndw - (n - 1)**windowovrl;
	...
	if( *lfft > MAXLFFT ){
		fprintf( stdout, "Error fft size greater than the largest allowed of %ld .\n", 
		 MAXLFFT );
		}
	else{
		fprintf( stdout, "Window size: %ld  Overlap: %ld  FFT size: %ld \n", 
