...
long int *nerr;
{
	...
	static double slice = 1.0;
        ...
        static long lmean = TRUE;
        float fwindow, fslice;
	static char type[4] = "mem";
        static char imagetype[6] = "color";
	static long order  = 200;
        static long morder = 100;
        static long lorder = FALSE;
        static long lbinary = FALSE;
        static long lcbar = TRUE;
        static long sfft = 1024;

        static long cnumber = 1;               /* number of correlation windows */
        static long lcnumber = FALSE;          /* was cnumber set by user? */

        static float cwinlength = 0.0;         /* length of correlation window (in seconds)
                                                  should default to the window size of the image (WINDOW) */
        static long lcwinlength = FALSE;       /* was cwinlength set? if not default to value of WINDOW */

        static char cwintype[9] = "HAMMING ";  /* correlation window type */
        long cwindex;

	/* prewhiten before computing correlation function? */
/*      static long lprew  = FALSE;
        static long nprew  = MPREWH + 1;  */

        static char scale[11] = "STOCHASTIC";  /* type of scaling */

	float *const Deltalist = &deltalist[0] - 1;
	long *const Nptslist = &nptslist[0] - 1;
