...
#include "specdata.h"
long /*FUNCTION*/ spectrogram(window, sliceint, type, order, nfiles, 
	 filelength, delta, specindex, specwidth, speclength, sfft,
         cwinlength, lcnumber, cnumber, cwintype, scale)
double window, sliceint;
char *type;
long int *order, nfiles, filelength[];
double delta;
long int *specindex, *specwidth, *speclength, sfft;
float cwinlength;
long lcnumber, cnumber;
char *cwintype;
char *scale;
{
	char windowfunc[11];
	long int buffersize, bufindex, done, err, err1, err2, idx, iorfft, 
	 length, lfft, nptswndw, pfile, spectrogram_v, windowovrl;
	float signals[MAXLFFT];
        float ridge_regress = .00001;

	long *const Filelength = &filelength[0] - 1;
	...
	if( calcfftsize( delta, sliceint, window, &iorfft, &lfft, &nptswndw, 
	 &buffersize, &windowovrl ) != 0 ){
		...
		allamb( &cmmem, (long)(1.5 * length), specindex, &err1 );
		...
		if( !err1 && !err2 ) {
			...
			*speclength = pfile/ *specwidth;
			fprintf( stdout, "Spectrogram dimensions are %ld  by %ld .\n", 
