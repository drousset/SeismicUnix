...
long int *nerr;
{
	...
	long int ixstart, ixstop, iystart, iystop, jfile, ncret, 
	...
	*nerr = 0;
	...
	for( jfile = 1; jfile <= nfiles; jfile++ ){
		...
		ixstart = max( 1, (long)( (xstart - xminimum)/cmgem.xdelta ) + 1 );
		ixstop = min( nxsize, (long)( (xstop - xminimum)/cmgem.xdelta ) + 2 );
		...
		iystart = max( 1, (long)( (ystart - yminimum)/cmgem.ydelta ) + 1 );
		iystop = min( nysize, (long)( (ystop - yminimum)/cmgem.ydelta ) + 2 );
