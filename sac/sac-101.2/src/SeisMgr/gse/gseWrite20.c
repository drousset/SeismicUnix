...
long stringMatch ( char *first , char *second , int num )
{
	int fiLen, seLen , idx;
...
void GetSiteInfo ( struct siteList *si , char *sta , long ondate ,
		   long offdate, float *lat , float *lon , float *elev )
{
    *lat = *lon = *elev = -999.0 ;
...
void GetWfdiscInfo ( struct wfdiscList *wf, char *sta , char *chan ,
		     long ondate , long offdate , char * inst ) 
{
    strcpy ( inst , "-" ) ;
...
int PutWaveformData ( FILE *ptr , DBlist tree , long cm6 )
{
    char id	[ 5 ] ,
	time	[ 24 ] ,
	station	[ 6 ] ,
...
int WriteGSEFile( char *outFile, DBlist tree, char *datasource, long cm6 )
{
    int Ntraces , nerr ;
    FILE *ptr ;
