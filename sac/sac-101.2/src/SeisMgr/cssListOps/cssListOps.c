...
static struct trace *dblCopyTraceData(struct trace *old, long nsamp);
...
static long dblIndex = 0;

static long dblNextIndex(void)
{
   dblIndex++;
   return dblIndex;
}  
...
struct trace * dblGetTrace ( long int index ) 
{
	int idx ;
	...
	for ( idx = 0 ; idx < index , wfL ; idx++ , wfL = wfL->next )
	{ /* do nothing */ } 
...
static void StreamData(ComplexFloat *data, long dataLen, FILE *fptr)
{
   int itemsWritten = 0;

   if(!data)
      dataLen = 0;
   itemsWritten += fwrite( (char *) &dataLen, sizeof(long), 1, fptr);
...
static void ReadComment(FILE *fptr, char *comment, long dataLen)
{
   int itemsRead = 0;
...
static void ReadLabelAndBytes(FILE *fptr, char *label, long *dataLen )
{
   int itemsRead = 0;

   itemsRead += fread( (char *) label, sizeof(char), 3, fptr);
   itemsRead +=	fread( (char *) dataLen, sizeof(long), 1, fptr);
...
static void ReadNumberOfCplxFloats(FILE *fptr,long *dataLen)
{
   int itemsRead = 0;
...
static void ReadIndex(FILE *fptr, long *index)
{
   int itemsRead = 0;

   itemsRead += fread( (char *) index, sizeof(long), 1, fptr);
   if( itemsRead < 1 ){
       dblSetError(1, "ERROR: Problem reading data from UserData file. \n");
   }
}
...
static void ReadComplexFloats(FILE *fptr, ComplexFloat *data, long dataLen)
{
   int itemsRead = 0;
...
int dblWriteUserData(DBlist list, char * filename)
{
   ...
   do{
	 ...
	 index = DC->reference;
	 fwrite( (char *) &index, sizeof(long), 1, fptr);
      }
   }while(DCstruct);
   ...
   fwrite( (char *) &Nrows, sizeof(long), 1, fptr);
