...
long gcCheckWftag ( struct wftag * wt , struct wfdiscList * wfL )
{
	long ID ;
...
long gcCheckSensor ( struct sensor * se , struct wfdiscList * wfL )
{
	char * ST , * CH ;
	double TI , ET ;
...
long gcCheckSitechan ( struct sitechan * sc , struct wfdiscList * wfL )
{
        char * ST , * CH ; 
        double TI , ET ;
...
long gcCheckSite ( struct site * si , struct wfdiscList * wfL )
{
        char * ST ;
        double TI , ET ;
...
long gcCheckInstrument ( struct instrument * in , struct sensorList * sL )
{
        long ID ;
...
long gcCheckEvent ( struct event * ev , struct wftagList * wtL )
{
	char *tagname ;
...
long gcCheckOrigin ( struct origin * orig , struct eventList * evL )
{
        long ID ;
...
/* If net in network matchs a net in any affiliation, return TRUE.
   If it does not match any of them, return FALSE */
/*
long gcCheckNetwork ( struct network * nw , struct affiliationList * afL )
{
        char * net ;
