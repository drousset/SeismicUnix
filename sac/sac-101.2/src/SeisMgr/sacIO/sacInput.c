...
long originNeeded   ( struct SACheader *header ) ;
long eventNeeded    ( struct SACheader *header ) ;
long siteNeeded     ( struct SACheader *header ) ;
long sitechanNeeded ( struct SACheader *header ) ;

static struct wftagList *FindWftag(DBlist tree, long wfid) ;
static struct wftagList *FindNxtWftag(struct wftagList *wt, DBlist tree, long wfid) ;
static struct wftagList *FindWftagByWfidAndTagname( DBlist tree, long wfid, char* tagname ) ;
static struct eventList *FindEvent(DBlist tree, long evid) ;
static struct originList *FindOrigin(DBlist tree, long orid) ;
static struct originList *FindOriginByEvid(DBlist tree, long evid) ;
...
int lngDefined(long value)
{
  if(value == longDef) return FALSE;
  return TRUE;
}
/* ------------------------------------------------------------------ */
...
static char *sacSetEtype(long ievtyp)
{
   switch(ievtyp){
...
static int HeaderOK(struct SACheader *header)
{
   ...
   if(header->iftype < IREAL || header->iftype > IUNKN || 
      header->iftype == longDef) return 0;

   if(header->idep == longDef)header->idep = IUNKN; 
   return 1;
}
/* ------------------------------------------------------------------ */
...
static sacSACdata *ReadSACdata(struct SACheader *header, long int index1, 
				                            long int index2)
{
...
sacSACdata *sacInput(char *filename, struct SACheader *header, ...)
{
    ...
    long int startIndex;    /* index to start reading in file */
    long int endIndex;      /* index of last pnt to read */
...
static struct sitechanList *FindSiteChan(DBlist tree, char *kstnm, char *kcmpnm, long jdate)
{
...
static struct sitechanList *MakeSiteChan(DBlist tree, char *kstnm, char *kcmpnm, float cmpinc, 
                                     float stdp, float cmpaz, long jdate)
{
...
static struct siteList *findSite(DBlist tree, char *kstnm, long jdate)
{
...
static int ArrivalJoinsToTrace(DBlist tree, struct arrivalList *ar, long wfid)
{
...
static void ResetAssocName(DBlist tree, long arid, char *NewSta)
{
...
static struct instrumentList *FindLinkedSensor(DBlist tree, long inid)
{
...
static struct originList *OriginByWfTagMatch(long wfid, DBlist tree)
{
   int evid = 0, prefor = 0 ;
   struct wftagList *wt = 0;
...
static struct wftagList *findArrivalWftag(DBlist tree, long arid)
{
   struct wftagList   *wt = 0;
   do{
...
int  AridJoinsWithWfid(DBlist tree, long arid, long wfid)
{
   struct wftagList   *wt = 0;
   do{
      wt = (struct wftagList *) dblNextTableInstance(wt, tree, dbl_LIST_WFTAG);
...
struct wftagList *FindConnectingWftag(DBlist tree, long arid, long wfid)
{
   struct wftagList   *wt = 0;
   do{
      wt = (struct wftagList *) dblNextTableInstance(wt, tree, dbl_LIST_WFTAG);
...
static struct assocList *findAssoc(DBlist tree, long arid)
{
   struct assocList *as = 0;
   do{
      as = (struct assocList *) dblNextTableInstance(as, tree, dbl_LIST_ASSOC);
...
static struct eventList *FindMatchingEventStruct(DBlist tree, long evid)
{
   struct eventList *ev = 0;
   do{
...
static struct siteList *sacAddSiteStruct(DBlist tree, char *name, struct SACheader *header, long jdate)
{
   struct siteList *si;
...
struct sacdataList *findSacData(DBlist tree, long wfid)
{
...
static void sacAddSacdataStruct(DBlist tree, struct SACheader *header, long wfid)
{
...
/*
static int isGoodEvid( DBlist tree, long evid, int ndfl )
{
   struct wftagList *wt = 0 ;
...
/*
static int isGoodPrefor( DBlist tree, long evid, int ndfl )
{
   struct wftagList *wt = 0 ;
   struct wfdiscList *wf = 0 ;
...
static struct wftagList *FindWftagByWfidAndTagname(DBlist tree, long wfid, 
                                                   char *tagname )
{
   struct wftagList *wt = 0;
   do{
...
static DBdataComm MatchingDataComment(DBlist tree, long Trcindex, int j)
{
   DBdataComm DCpntr;
