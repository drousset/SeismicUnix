...
static struct arrivalList *MatchingArrival(DBlist tree, char *sta, char *chan, double time, 
                                           double end, char *SACfield, long wfid)
{
...
struct arrivalList *sacFindNextMatchingArrival(struct arrivalList *ar, 
					       DBlist tree, char *sta, char *chan,
                                               double time, double end, long wfid)
...
static struct siteList *sacFindSite(DBlist tree, char *sta, long jdate, long wfid)
{
...
static struct originList *sacFindMatchingOrigin(DBlist tree, long orid)
{
...
static struct eventList *sacFindEvent(DBlist tree, long evid);
...
static struct assocList *sacFindAssoc(DBlist tree, long orid, char* sta)
{
...
static struct assocList *sacFindAssocFromArid(DBlist tree, long arid)
{
   struct assocList *as = 0;
...
static struct wftagList *sacFindWftag(DBlist tree, long wfid, char* tagname )
{
   struct wftagList *wt = 0;
...
static long sacSetEvType(char *etype)
{
...
{
  if(!CSSchrDefined(orig->element->auth) ){h->imagsrc = longDef; return;}
