...
static void FreeMatrix(ComplexFloat **matrix, long Nrows, long Ncols)
{
   int j;
...
static int WriteUserDataCommentStruct(ComplexFloat *value, long Npts, char* comment, 
                                      enum dataType Type, long Index, FILE* ptr)
{
   CSSBTAG tag;
   DataCom DC;
...
static int WriteUserDataMatrix(ComplexFloat **matrix, long Nrows, long Ncols, 
...
static int AddDCStruct(DBlist tree, CSSBTAG Tag, FILE* ptr)
{
   ...
   if( fread(data, DC.Datalen * sizeof(ComplexFloat), 1, ptr ) != 1)return 0;
   if( fread(comment, DC.CommentLen, 1, ptr ) != 1)return 0;
...
int ReadCSSBfile(const char *fname, const char *WorkSetName, int Replace, 
                 int MaxTraces, int verbose, double MaxPhysMem, long takeEvid )
{
