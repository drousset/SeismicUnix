...
long int NearestInt( double D )
{
   double Floor = floor(D);
   double Ceil  = ceil(D);
   return (Ceil - D > D - Floor) ? (long) Floor : (long) Ceil;
}
/* ------------------------------------------------------------- */
...
int IsNormalized( double calper, int nfreq, double delfrq, const double* xre, const double* xim )
{
   ...
   long int j = NearestInt( NormFreq / delfrq );
   if( j < 0 || j >= nfreq ){
...
double GetNormalizationFactor( int nfreq, double delfrq, const double* xre, const double* xim )
{
   ...
   */
   ...
   /* Contact the database to get Instrument Information 
      Non-Oracle Database implementations always return -999.0
      If you have no idea, it will return -999.0
    */
   ...
   */
   ...
   if( calper <= 0 ){
      NormStatus = Unknown;
      /* printf( "%s", "\tCalper is not available. ""
	 "Cannot tell if response is normalized, so transfer function will be used without scaling.\n");
      */
   }
   else{
   ...
   if( Unset(calib) ){
     /* printf( "%s", "\tCalib is not available. "
	"Cannot tell if scale factor has been applied on input, so transfer function will be used without scaling.\n");
     */
