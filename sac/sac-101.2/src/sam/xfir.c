...
{
	long lusrec;
	long int isym, j, j_, jb, jdfl, jdfl_, jf, jscmem, ncfir4, ncpfn, 
...
L_1000:
	...
	if( lusrec ){
		...
		for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
			...
			copy( (long*)cmmem.sacmem[ndxscr], (long*)cmmem.sacmem[ndx1], nlen );
		...
		for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
			...
			copy( (long*)cmmem.sacmem[ndx1], (long*)cmmem.sacmem[ndxsrl], nlen );
