...
void /* FUNCTION */ fdWhitenWrite( long int * memptr , char * kprefix ,
				   float * userData , long int newnpts ,
				   long int nFreq , long int * nerr )
{
	/* index sacmem for Amplitude, Phase, 
	   and the impulse response. */

	long int fileDescriptor = 0 , hdrindex , 
	...
	*lcalda = FALSE ;
	...
	for ( jdx = 0 ; jdx < 3 ; jdx++ ) {	/* loop between output files. */
		...
		*nerr = 301;
	    ...
	    copy ( (long*) cmhdr.fhdr , (long*) cmmem.sacmem[ hdrindex ] , MCMHDR );
	    zputc ( kmhdr.khdr[ 0 ] , 9 , (long int *)(cmmem.sacmem[ hdrindex ] + MCMHDR), ( MCPW + 1 ) * MKHDR ) ;
...
void aphdrw ( long int newnpts , long int nFreq )
{
        *nsnpts = newnpts ;
        *npts = nFreq ;
        *sdelta = *user6 ;
...
void gdhdrw ( long int newnpts , long int nFreq )
{
        *nsnpts = newnpts ;
        *npts = nFreq / 2 ;
        *sdelta = *user6 ;
...
void irhdrw ( long int newnpts , long int nFreq )
{
        *nsnpts = nFreq ;
        *npts = newnpts ;
        *delta = *user6 ;
