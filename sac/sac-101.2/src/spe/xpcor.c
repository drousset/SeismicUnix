...
{
	char kcorlb[ 17 ] ;
	long lany , lpcsec , lframs , lprint = FALSE , ltry = FALSE ;
	long int ifidls , nlnplt , notused ;
	float pcsec , xjunk ;
	...
	*nerr = 0;
	...
	if( lpcsec )
	    nlnplt = min( cmspe.nlncor/2 + 1, (long)( pcsec*cmspe.samfrq ) );
	else
	...
	if( cmspe.lspeid ){
	    ...
	    /* -- Number of correlation windows. */
            sprintf(kcorlb,"NUMBER %3ld", cmspe.numwin );
	    ...
	    /* -- Prewhitening flag. */
	    if( cmspe.nprewh > 0 )
                sprintf(kcorlb,"PREWHITEN %3ld", cmspe.nprewh );
	    else
