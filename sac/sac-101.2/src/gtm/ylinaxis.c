...
void /*FUNCTION*/ ylinaxis(llefax, lrigax, lleftc, lrigtc, widlef, 
	 widrig)
long llefax, lrigax, lleftc, lrigtc;
float *widlef, *widrig;
{
	char khorz[9], ktemp[9], kvert[9];
	long lneg, lneglb, lpower, lrev, lygrid, lypowr;
	long int ia, ib, igdlog, iline, iygrid, jdiv, jdiv_, jpower, jstep, 
	...
	if( grdlog*divlog >= 0. ){
		...
		if( labs( igdlog ) >= 3 && lypowr ){
			mds = 0;
			nds = max( 4, (long)( grdlog ) - (long)( divlog ) + 2 );
