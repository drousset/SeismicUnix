...
{
	char kne[9], kstnm1[9];
	long int ic1, ic2, eexact, iennd1, ibeg , dbeg, iftyp1, jdx, jbfl, jdfl, jdfl_, n1dttm[6], 
	 nbdttm[6], ndx, ndx1o, ndx2o, ndx1n, ndx2n,
	 ndxbfl, ndxnew, ndxold, nedttm[6], nlen, nlnbfl, nlnnew, nlnold,
	 npts1, ntused, nzeros[MDFL], icomORroll;
	float begin1, delta1, diff, ennd1, dennd1 , temp ;
	static long lchecktimes = TRUE;

	long *const N1dttm = &n1dttm[0] - 1;
	long *const Nbdttm = &nbdttm[0] - 1;
	long *const Nedttm = &nedttm[0] - 1;
	long *const Nzeros = &nzeros[0] - 1;
	...
	*nerr = 0;
	...
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
		...
		*nerr = 1801;
	    ...
	    iennd1 = (long) ennd1 ;	/* integer portion of end time */
	    ...
	    ibeg = (long) (*begin) ;	/* integer portion of begin */
		...
		Nzeros[jdfl] = (long)( (diff - 0.5**delta)/ *delta ) + 1;
		setmsg( "WARNING", 1805 );
		lnumcl( kmdfm.kdfl,MAXCHARS, jbfl, &ic1, &ic2 );
                apcmsg2(&kmdfm.kdfl[ic1 - 1],ic2-ic1+1);
		lnumcl( kmbom.kbfl,MAXCHARS, jbfl, &ic1, &ic2 );
...
/*		getfil( jdfl, TRUE, &nlen, &ndx1o, &ndx2o, nerr );
		getbfl( jbfl, TRUE, &nlen, &ndx1n, &ndx2n, nerr );
*/
		for ( idx = ( *begin - begin1 ) / *delta ,
		      kdx = 0 ;
		      idx <= (  ennd1 - begin1 ) / *delta ;
		      idx++ , kdx++ )
		{
	...
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
	    ...
	    /* -- Copy old data to beginning of new block. */
            copy((long*)cmmem.sacmem[ndxold],(long*)cmmem.sacmem[ndxnew],nlnold);
	    ndx =  nlnold;
		...
		copy( (long*)cmmem.sacmem[ndxbfl], (long*)(cmmem.sacmem[ndxnew]+ndx), nlnbfl );
	    }

	    /* -- Case of Overlap */
	    else if ( Nzeros[ jdfl ] < 0 ) {
		/* Append second data series except for overlap. */
		copy( (long*)(cmmem.sacmem[ndxbfl] - Nzeros[ jdfl ]),
		      (long*)(cmmem.sacmem[ndxnew]+ndx), nlnbfl + Nzeros[ jdfl ] );
		nlnnew-- ;
	    }
