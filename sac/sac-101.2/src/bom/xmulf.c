...
void /*FUNCTION*/ xmulf(nerr)
long int *nerr;
{
	char kdttm[33];
	long int ic1, ic2, jdx, jbfl, jdfl, n1bdtm[6], n1zdtm[6], 
	 n2bdtm[6], ncerr, ndx1, ndx1b, ndx2, ndx2b, nlen, nlenb, npts1;
	long lnewhdr ; /* let header data come from new file */
	float begin1, delta1, delta2, diff, value;

        float *Sacmem1, *Sacmem2;

	/*=====================================================================
	 * PURPOSE:  To execute the action command MULF.
	 *           This command multiplies a set of files by data in memory.
	 *=====================================================================
	 * OUTPUT ARGUMENTS:
	 ...
	 *    920331:  Added msg about BINOPEER cmd when header delta differs.
	...
	*nerr = 0;
	...
	for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
		...
		*nerr = 1306;
		...
		lnumcl( kmbom.kbfl,MAXCHARS, jbfl, &ic1, &ic2 );
		apcmsg2(&kmbom.kbfl[ic1 - 1],ic2-ic1+1);
		aplmsg( "- can be controlled using the BINOPERR command" ,47 );
