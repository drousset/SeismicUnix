...
double hrToEpoch(int, int, int, int, int, float, long *) ;
void doyToMD () ;
int isLeapYear () ;
long mdyToJul() ;
...
long int *nerr;
{
	...
	float zsec ;
	long lexist, unused1, unused2, unused3 ;
	float rvalue;
	double value ;
...
double hrToEpoch( int year, int month, int day, int hour,
                    int min, float second , long int * nerr ) {
   int err;
   int *et = eomnth[isLeapYear(year)];
   long basedate = 2440588; /* astronomical julian day for 01/01/1970 */
   long julianDay;   /* astronomical julian day */
   long DeltaDays;   /* number of days relative to basedate */
...
long mdyToJul(int mm, int id, int iyyy)
{
        long jul;
        int ja,jy=iyyy,jm;
        long IGREG = 588829;
