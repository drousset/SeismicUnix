...
#include "gdm.h"
#include "gem.h"
void /*FUNCTION*/ begindevices(devices, devices_s, ndevices, nerr)
char *devices;   int devices_s;
long int ndevices, *nerr;
{
#define DEVICES(I_,J_)	(devices+(I_)*(devices_s)+(J_))
	char ktype[9];
	long turnon[MGD];
	long int igd, jdevice, jgd;
	void begindevice3(), begindevice4(), enddevice3(), enddevice4();
        void begindevice5(), enddevice5();

	long *const Turnon = &turnon[0] - 1;
