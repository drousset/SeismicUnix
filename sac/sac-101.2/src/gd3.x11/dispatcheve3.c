...
/*  Process each event */
	...
	if (is_basew) {
	  ...
	  if ((basew_info.width != basew3[index].width_p) ||
	    ...
	    XCopyArea(display3, plotw3[index].win, plotw3_pixmap, plotw3[index].gc,
		      0, 0,
		      basew_info.width,
		      basew_info.height,
		      0, 0);
	  ...
	  /* Brian Savage <savage13@dtm.ciw.edu> 2005/02/13 
	     Added to tell X11 to draw in window just after creating the window 
	  */
	  expose3();
	...
	if (is_plotw) {
	  if(event.xexpose.count == 0) {
	    XClearWindow(display3, plotw3[index].win);
	    XCopyArea(display3, plotw3_pixmap, plotw3[index].win, plotw3[index].gc,
		      0, 0, 
		      plotw3[index].width_p,
		      plotw3[index].height_p,
		      0, 0);
	  }
	}
#endif /* USE_X11_DOUBLE_BUFFER */
	...
	case MotionNotify:
		   ...
		   if(use_massive_crosshairs) {
		     ...
		     XCopyArea(display3, plotw3_pixmap, plotw3[index].win, plotw3[index].gc, 
                               ...
                               plotw3[index].height_p, 
                               0, 0); 
		     XDrawLine(display3, plotw3[index].win, plotw3[index].gc,
                               0, current_mouse_y_p3 + owoffset,
                               plotw3[index].width_p - 1, current_mouse_y_p3 + owoffset);
                     XDrawLine(display3, plotw3[index].win, plotw3[index].gc,
                               current_mouse_x_p3 + owoffset, 0,
