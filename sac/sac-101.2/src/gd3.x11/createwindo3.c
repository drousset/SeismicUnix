...
createwindow3(win_num, xmin_vp, xmax_vp, ymin_vp, ymax_vp, nerr)
  int *win_num, *nerr;
  float *xmin_vp, *xmax_vp, *ymin_vp, *ymax_vp;
...
/* Create title window and set attributes */
  ...
  XSelectInput(display3,titlew3[*win_num].win,
...
/* Create plot window and set attributes */
  ...
  /* A Double Buffer to store the image
   Drawing will be done on the image then copied to the screen
   Brian Savage <savage13@dtm.ciw.edu> 2005/02/13
  */
  plotw3_pixmap = XCreatePixmap(display3, basew3[*win_num].win,
				width_p, height_p,
				DefaultDepth(display3, DefaultScreen(display3)));

  fill_background3(*win_num);
...
/* Map the windows */
  ...
  XFlush(display3);
  XSync(display3,True);
#endif /* NOT USE_X11_DOUBLE_BUFFER */
