...
#define AI(I_,J_)	(*(ai+(I_)*(nm)+(J_)))
	long int i, i_, ii, ii_, j, j_, jp1, k, k_, l;
...
struct t_iop {
	long int ddt, err, fmt, lct[4], lin[1024], lpt[6], hio, rio, wio, 
	 rte;
        FILE *wte;
	}	iop;
...
double a, b;
{
	...
	long *const Lct = &iop.lct[0] - 1;
	long *const Lin = &iop.lin[0] - 1;
	long *const Lpt = &iop.lpt[0] - 1;
...
#define Z(I_,J_)	(*(z+(I_)*(nm)+(J_)))
	long int i, i_, ii, ii_, j, j_, k, k_, l, l1, l_, m, m_, mml;
