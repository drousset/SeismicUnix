...
void /*FUNCTION*/ pltplr(s, naz, ninc, levels, scalng, lprint, label, label_s, 
	 title, title_s)
float s[];
long int naz, ninc, levels;
long lprint ;
char *scalng, *label;   int label_s;
char *title;   int title_s;
{
	long exists, lany, lfullsav;
	long int _l0, i, i_, iend_, lenlab, nerr, np, ptr;
...
L_4:
	...
	if( ptr > lenlab )
        ...
        if(csave != NULL ) iend_ = (long)(csave - label + 1);
