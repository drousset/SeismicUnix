...
{
	char kfile[MCPFN+1];
	long int iseed, j, j1, jcmp, jdfl, junk, ndx1, 
	...
	*nerr = 0;
	...
	if( cmexm.ifgtp == 9 ){
		...
		cmdfm.ndfl = min( MDFL, (long)( Fgraco[1] + 0.1 ) );
		jcmp = 1;
		for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
			...
			if( jdfl <= 9 ){
                                sprintf(kfile,"%s%1ld","RANDOM0", jdfl );
			}
			else{
                                sprintf(kfile,"%s%2ld","RANDOM", jdfl );
			}
...
L_2120:
	...
	/* subtracted 1 from fgistr to convert 
	   ...
	   PG 1/4/01. */
        Sacmem = cmmem.sacmem[ndx1];
        for( j = 0; j <= nra-1; j++){
          if(((long)(cmexm.fgistr[j]+RNDOFF) < *npts) &&
             ((long)(cmexm.fgistr[j]+RNDOFF) > 0))
           *(Sacmem + (long)(cmexm.fgistr[j]-1+RNDOFF)) = 1.0;
	}
