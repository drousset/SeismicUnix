...
void /*FUNCTION*/ updatedfl(call_data, update, nerr)
sac_files call_data;
long update;
long *nerr;
{
        char kline[MCMSG+1], kfile[MCPFN+1];
        long jdfl, ndxh, ndx1, ndx2, i, ndflsave;
	...
	*nerr = 0;
	    ...
	    setmsg("OUTPUT", 0);
	    sprintf(kline,"%s%3ld%s","Adding ", call_data.nfiles ,
	    ...
	    *nerr = 6010;
	    ...
	    if( i <= 9 ){
		sprintf(kfile,"%s%1ld", "EXTERN0", i );
	    }else{
		sprintf(kfile,"%s%2ld", "EXTERN",  i );
	    }
	    ...
	    memcpy(cmhdr.nhdr, this_header->ext_nhdr, MNHDR*sizeof(long));
	    memcpy(cmhdr.ihdr, this_header->ext_ihdr, MIHDR*sizeof(long));
	    memcpy(cmhdr.lhdr, this_header->ext_lhdr, MLHDR*sizeof(long));
