...
void /*FUNCTION*/ readsuds( lmore, kdirin, kdirin_s, kdflin, 
...
long isASCII;
float MaxMem;
long int ndflin, *nerr;
{
   long filesReturned = 0;
   DBlist tree;
   int Replace        = 0;
   ...
   int j;
	...
	/*=====================================================================
	 * PURPOSE:  To read a set of SUDS files into memory.
	 *=====================================================================
	 * INTPUT ARGUMENTS:
	...
	*nerr = 0;
	...
	SUDSFileList = MakeFileList(kdirin, kdflin, &Nfiles);
	for(j=0;j<Nfiles;j++){
           filesReturned += sudsToCSSlist(SUDSFileList[j], WorkSetName, Replace, 
	   ...
	   if(smFracPhysMemUsed() > MaxMem)
        ...
        if(Verbose)printf("\n");
        FreeStringArray(SUDSFileList,     Nfiles);
