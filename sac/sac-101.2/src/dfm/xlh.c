...
long int *nerr;
{
	char kerase[41], kline[MCMSG+1], kresp[9], krpttx[MRPT][41], ktok[9], 
	 kwait[9];
	long lwait, lwdes, lwname, lwundf;
	long int ic1, ic2, j, j_, jdfl, jrpt, jrpt_, jrpttx, jrpttx_, 
	...
	*nerr = 0;
	...
	if( *nerr != 0 ) {
	    	cmhdr.llh = FALSE ;	/* no longer executing xlh(). maf 961212 */
		return;
	}
	...
	if( jsprpt > 0 )
        ...
        if( ldef ) {
		cmhdr.llh = FALSE ;	/* no longer executing xlh(). */
		return;
        }
