...
long int *nerr;
{
	...
	long int icomORroll ;
	long lbcksp, lexpnd, lfree, lmore, ltoend, numchar,
	     lprint = FALSE , ltry = FALSE ;
	long int idx, ic, ic1, ic2, icpntr, idflc1, idflc2, iopch[MAXCH], 
	 ipt, itype, jdx, jch, jcomp, jdfl, jen, nc, nc1, nc2, ncerr,
	 nchar, ndcont, ndflin, ndflrq, ndflsv, ndform, ndxch[MAXCH-(0)+1],
	 nentry, newlen, newndx, nlnch[MAXCH-(0)+1], notused, nptch[MAXCH-(0)+1], nsndfl, numch, numxch, numych ;
        FILE *nun;
	...
	/*=====================================================================
	 * PURPOSE:  To execute the action command READALPHA.
  ...
  malloc_verify();
		...
		for( jen = 1; jen <= nentry; jen++ ){
			...
			if( nptch[jch] >= nlnch[jch] ){
			    ...
			    if( *nerr != 0 )
				goto L_8888;
			    copy( (long*)cmmem.sacmem[ndxch[jch]],
				  (long*)cmmem.sacmem[newndx], nptch[jch] );
...
L_5000:
		...
		for( jch = 2; jch <= numych; jch++ ){
		    ...
		    Ncomp[jdfl] = 2;
		    copy( (long*)cmmem.sacmem[ndxch[0]],
			  (long*)cmmem.sacmem[cmdfm.ndxdta[jdfl - 1][1]],
			  Nlndta[jdfl] );
		}
	    }
