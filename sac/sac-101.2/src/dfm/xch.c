...
long int *nerr;
{
	...
	long lallt, lfound, lhdrc[MLHDR], lnumbr, log;
	long int icat[MHDR], icatx, igdttm, ihdrc[MIHDR], item[MHDR], 
	...
	/* variables added to allow file specification, maf 960812 */
        long	 doFile[MDFL] ;	/* 1 if the corresponding file is to be modified */
	int	 idx ;		/* index doFile during initialization */
        float	 rnum ;		/* number passed back from lctok */


	float *const Fhdrc = &fhdrc[0] - 1;
	long *const Icat = &icat[0] - 1;
	long *const Ihdrc = &ihdrc[0] - 1;
	long *const Item = &item[0] - 1;
	long *const Lhdrc = &lhdrc[0] - 1;
	long *const Nhdrc = &nhdrc[0] - 1;
...
L_8888:
	 ...
	 *    900517:  Fixed VAX bug in changing longer header character variables.
