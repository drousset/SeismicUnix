...
#include "com.h"
#include "dfm.h"
void /*FUNCTION*/ xrcss(nerr)
long int *nerr;
{
	char _c0[2], kvers_in[9];
        static char kdflin[MCMSG+1], kdflinsave[MCMSG+1];
	long llocal, lmore, lshift, lscale, larray ;
	long int jcdflbeg, jcdflend, jcparmbeg, nchar, icomORroll;
        static long ndflin;
        static long Verbose = 0;
        static long ibinORasc;

	char kmag[4] ;	/* magnitude type: mb, ms, or ml. maf 970206 */

	/*=====================================================================
	 * PURPOSE:  To execute the action command READCSS.
	 ...
	 *    970403:  New option to conveniently select the channel.
	 *    970206:  New option to specify which magnitude to read.  maf
	 *    961216:  Now filtering on station rather than gain.  maf
	 *    920420:  Changed null string, "", to ' ' - porting to IBM. 
	 *    910703:  add # to parameter keyword GAIN; 
	...
	*nerr = 0;
	...
	/*	if ( lscale ) {
		...
		{ /* do nothing */ }
		    ...
		    cfmt("Duplicate DFL, check spelling of prior keywds:$",48 );
		    cresp();
		    return ;
		}
	 ...
	 *    ../../inc/dfm */
	readcfl( lmore, kmdfm.krdcssdir,MCPFN+1, kdflin,MCMSG+1, ndflin,
