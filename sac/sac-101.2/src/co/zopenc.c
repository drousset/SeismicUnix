...
long *pnewfl;		/* create if necessary flag */
long *pro;              /* read-only flag */
int *pnerr;		/* address to store error code */
long pfnlen;            /* length of file name from F77 */
{
    int i;
    int mode;
    int errno;
    ...
    *pnerr=0;
	...
	mode=O_RDWR;
    if ( pfnlen > (long int) strlen( pfname ) || pfnlen < 0) {
...
zopenc_(pfd,pfname,pnewfl,pro,pnerr,pfnlen)
int *pfd;               /* address to store file descriptor */
char pfname[];          /* file name */
long *pnewfl;           /* create if necessary flag */
long *pro;              /* read-only flag */
