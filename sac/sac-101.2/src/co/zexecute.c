...
void /*FUNCTION*/ zexecute(index, nerr)
long int index, *nerr;
{
	long i, jdfl, nlen, ndx1, ndx2, ireturn;
        sac_files call_data;
	...
	*nerr = 0;
	...
	if( index <= cmextcom.nfiles ){
          ...
          for( jdfl = 1; jdfl <= cmdfm.ndfl; jdfl++ ){
            ...
            memcpy(this_header->ext_nhdr,cmhdr.nhdr,MNHDR*sizeof(long));
            memcpy(this_header->ext_ihdr,cmhdr.ihdr,MIHDR*sizeof(long));
            memcpy(this_header->ext_lhdr,cmhdr.lhdr,MLHDR*sizeof(long));
