...
void
znfilef(long int *nfu,
	char      *kname,     /* File name to open */
	char      *ktype,     /* 'DATA' or Error */
	long int  *nlen,      /* Unused, historical from Fortran on MASSCOMP */
	long int  *nerr,      /* Error return Flag */
	long int   kname_s,   /* Length of string kname */
	long int   ktype_s) { /* Length of string ktype */
  znfile(nfu, kname, kname_s, ktype, ktype_s, nerr);
}

void
znfilef_(long int *nfu,
	 char      *kname,
	 char      *ktype,
	 long int  *nlen,
	 long int  *nerr,
