...
typedef struct {
   long  fstype;               /* Structure types or identifiers */
   char  *fname;               /* Name used to identify structure item */
   long  ftype;                /* Type of variable in field */
   long  flength;              /* Length of variable in the field */
   long  offset;               /* Offset of variable pointer from beginning of */
   ...
   long  nextfstype;           /* If structure, this is fstype */
} SUDS_FORM;
...
typedef struct {
   ...
   long len_struct;           /* structure length in bytes for fast reading */
                              /* and to identify new versions of the structure */
   long len_data;             /* length of data following structure in bytes */
} SUDS_STRUCTTAG;
...
typedef struct {
   ...
   long   begint;             /* time this calibration becomes effective */
   long   endt;               /* time this calibration is no longer effective */
} SUDS_CALIBRATION;
#define CA_LEN 508
typedef struct CA{
  SUDS_CALIBRATION *ca;
...
typedef struct {
   ...
   long active;              /* Set is defined after this time */
   long inactive;            /* Set is not defined after this time */
} SUDS_CHANSET;
#define CS_LEN 21
...
typedef struct _CHANSETENTRY {
   long  inst_num;           /* Instrument serial number */
...
typedef struct {
   ...
   long  length;             /* number of samples in trace */
   ...
   long  numclip;            /* number of clipped datapoints */
...
typedef struct {
   ...
   long  event_number;       /* unique event number assigned locally. */
   long  spareL;             /* spare */
} SUDS_DETECTOR;
#define DE_LEN 24
...
typedef struct {
   ...
   float   frequency;        /* sensor corner frequency, vco freq, transmitter */
                             /* frequency, etc. */
   long effective;           /* date/time these values became effective */
} SUDS_EQUIPMENT;
#define EQ_LEN 60
...
typedef struct {
   ...
   long  number;            /* unique event number assigned by organization */
...
typedef struct {
   ...
   long time_of_pick;        /* time this pick was made */
...
typedef struct {
   ...
   long  void_samp;           /* Invalid or void sample value */
   ...
   long effective;            /* Time these setting took effect */
...
typedef struct {
   ...
   long  beginloc;            /* bytes from begining of file to trace */
} SUDS_LOCTRACE;
#define LO_LEN 24
...
typedef struct {
   ...
   short  spareG;            /* spare */
   long  numsamps;           /* number of sample sweeps. Typically not known */
                             /* when header is written, but can be added later */
   long  blocksize;          /* number of demultiplexed samples per channel if */
...
typedef struct {
   ...
   long  region;             /* geographic region code assigned locally */
   ...
   long effective;           /* time this solution was calculated */
} SUDS_ORIGIN;
#define OR_LEN 100
...
typedef struct {
   long  event_num;           /* unique event number */
...
typedef struct {
   ...
   char    program;           /* program used to decode time: */
                              /* e = irige, c = irigc */
   long effective_time;       /* time this correction was calculated */
   short  spareM;
} SUDS_TIMECORRECTION;
#define TC_LEN 32
...
typedef struct {
   ...
   long time_effective;      /* time this model was created */
} SUDS_VELMODEL;
#define VM_LEN 48
