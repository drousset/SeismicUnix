...
/* GARNISH: $Revision: 1.1 $ ; $Date: 2004/11/12 17:04:52 $	*/
     ...
     XColor			color;
{
  ...
  XClearArea(display, window, x,y,textwidth*strlen(text)+6,textheight + 6,False);
  ...
  i = XAllocColor(display, colormap, &generic);
  if (style == RELATIVE)
    w = textwidth*strlen(text);
  else
    w = fixed;
  if (state == UP) {
    ...
    XDrawImageString(display, window, gc, x+3,y+textheight+3,text,strlen(text));
  }
  else {
    XSetForeground(display, gc, generic.pixel);
    XFillRectangle(display, window, gc, x+3, y+3,(w), textheight+12);
    ...
    XDrawImageString(display, window, gc, x+4,y+textheight+3,text,strlen(text));
  }
  XSync(display,0);
  XFlush(display);
}
	  ...
	  int c_height, int mode)
{
  ...
  if (((int) strlen(text)) > 0) {
    ...
    XDrawString(display, win, gc, x+tx,y+ty,text,strlen(text));
  }
...
*/
  l = strlen(&set->text[which][set->view_start[which]]);
...
void CheckTextSet(TextSet *set, int x, int y)
{
  ...
  if (y > set->starty-set->char_height && y < set->starty)  {
    ...
    if (set->char_pos > (((int) strlen(set->text[set->currently_pointed]))-1))
      set->char_pos = strlen(set->text[set->currently_pointed]);
    if (i >= 0)
...
TextSet *CreateTextSet(Window wind, int startx, int starty, int number,
		       ...
		       int char_height, unsigned int foreground,
