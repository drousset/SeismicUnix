			...
			*eqptr = (char)0;
			...
			/* Debugging dump     */
     			  /* fprintf(stderr,     
     			      ...
     			      nargs,argtbl[nargs].name
                              ,argtbl[nargs].asciival);     
				*/
                        }
			...
			nargs++;
             ...
             for ( j = 0; parlist[j].name != 0; j++) {
                        if( !(strcmp( parlist[j].name ,argtbl[i].name )) ){
                              goto OK;
                        }
             }

             sprintf( errstring ,"Bad parameter: %s\n",argtbl[i].name );
             err( errstring );
             ...
             if( parlist[j].type == 'i' ){
                ...
                if( !sscanf( argtbl[i].asciival ,"%d" ,&ival ) ){
                   err( "bad ascii value" );
                }
                sscanf( parlist[j].min ,"%d" ,&imin );
                sscanf( parlist[j].max ,"%d" ,&imax );
                if( ival < imin || ival > imax ){
                   err( "value out of range" );
                }

             }else if( parlist[j].type == 'f' ){
                if( !sscanf( argtbl[i].asciival ,"%f" ,&fval ) ){
                   err( "bad ascii value" );
                }
                sscanf( parlist[j].min ,"%f" ,&fmin );
                sscanf( parlist[j].max ,"%f" ,&fmax );
                if( fval < fmin || fval > fmax ){
                   err( "value out of range" );
                }
             }else if( parlist[j].type == 's' ){
                /* can't really check this */

             }else{
                err( "bad parlist type" );
