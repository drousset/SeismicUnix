...
/*****************************************************************************
Functions to manipulate 2-dimensional matrices that are too big to fit
in real memory, but that are small enough to fit in virtual memory:
...
void
bmfree (bmstate *state)
/*****************************************************************************
free a big matrix
******************************************************************************
Input:
state		pointer to big matrix state (returned by bmalloc)
******************************************************************************
Author:		Dave Hale, Colorado School of Mines, 05/17/89
*****************************************************************************/
{
...
void
bmread (bmstate *s, int d, int k1, int k2, int n, void *v)
/*****************************************************************************
read a vector from a big matrix
******************************************************************************
Input:
s    		pointer to big matrix state (returned by bmalloc)
...
void
bmwrite (bmstate *s, int d, int k1, int k2, int n, void *v)
/*****************************************************************************
	...
	int *nbpe, char **q, int *qstep, char **p, int *pstep, int *i, int *l)
{
	...
	if (d==1) {
		*nbpe = nb;
		*p = s->p+nb*(i2*l1+j1*l1*l2+j2*l1*l2*m1);
		*pstep = nb*l1*l2;
		*qstep = nb;
		*q = *p+i1*(*qstep);
		*i = i1;
		...
		*nbpe = nb;
		*p = s->p+nb*(i1+j1*l1*l2+j2*l1*l2*m1);
		*pstep = nb*l1*l2*m1;
		*qstep = nb*l1;
		*q = *p+i2*(*qstep);
		*i = i2;
