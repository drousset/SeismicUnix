...
main(int argc, char **argv)
{
   ...
   /* optional parameters */
   if (!getparint("tord", &tord))
      tord = 0;
   if (!getparint("ghdout", &ghdout))
      ghdout = 1;

   /* read velocity grid header */
   ierr = fgetusghdr(infp, &usgh);
   if (ierr != 0)
      err(" input vavg grid not standard grid file format \n");
      ...
      for (i2 = 0; i2 < n2; i2++) {
         ...
         for (i1 = 1; i1 < n1; i1++) {
            ...
            if (gmax < vint[i1])
               gmax = vint[i1];
            if( vint[i1] < 0 ){
               fprintf( stderr ,"negative velocity\n" );
            }
         }
   ...
   if (ghdout == 1) {
      ...
      if (ierr != 0)
         err(" output vint grid file error \n");
   }
