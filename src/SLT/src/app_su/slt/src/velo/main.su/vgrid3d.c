...
main(int argc, char **argv)
{
   ...
   if (!getparfloat("dvs", &dvs))
      err(" dvs missing ");
   /* optional parameters */
   if (!getparint("ivs", &ivs))
      ivs = 0;
   if (!getparint("vitype", &vitype))
      vitype = 0;
   if (vitype != 0 && vitype != 1)
      err("check vitype");
   if (!getparint("votype", &votype))
      votype = 1;
   if (!getparint("smx", &smx))
      smx = 1;
   if (!getparint("smy", &smy))
      smy = 1;
   if (!getparint("sms", &sms))
      sms = 1;
   if (!getparfloat("vscale", &vscale))
      vscale = 1.0;
   if (!getparfloat("vmin", &vmin))
      vmin = 1000.0;
   if (!getparfloat("vmax", &vmax))
      vmax = 100000.0;
   if (!getparint("tslice", &tslice))
      tslice = 1;
   if (!getparint("vintpr", &vintpr))
      vintpr = 0;
   if (!getparint("intype", &intype))
      intype = 1;
   if (!getparint("verbose", &verbose))
      verbose = 1;
   if (!getparint("nf", &nf))
      nf = 3;
   if (!getparfloat("dismax", &dismax))
      dismax = 10000.;
   if (!getparint("rminv", &rminv))
      rminv = 0;
   if (!getparint("ocdp2", &ocdp2))
      ocdp2 = 0;
   if (!getparint("dcdp2", &dcdp2))
      dcdp2 = 0;
   if (!getparint("oline3", &oline3))
      oline3 = 0;
   if (!getparint("dline3", &dline3))
      dline3 = 0;
   if (!getparfloat("tlastpick", &tlastpick))
      tlastpick = -99999.;
   if (!getparfloat("vlastpick", &vlastpick))
      vlastpick = -99999.;
   if (!getparint("noxyintp", &noxyintp))
      noxyintp = 0;
   ...
   /* if input and output velocity are the same, skip the step to convert
      the velocity to interval velocity first */
   if (votype == vitype) {
      votype = 1;
      vitype = 1;
   }

   /* at most 4096 input (x,y) VS3D cards with at most 256 time-vel pairs
      each */
   ...
   /* 
      for(i=0;i<nxy;i++) { for(j=0;j<nps[i];j++) fprintf(stderr,"x=%f y=%f
      t=%f v=%f \n", xs[i],ys[i],tpicks[j+i*n1],vpicks[j+i*n1]);
      fprintf(stderr,"\n"); } */

   if (nxy == 0)
      err("No VS3D card input ! Job aborted");

   if (rminv > 0)
      removeinv(tpicks, vpicks, nps, nxy, n1, rminv);

   /* find out the maximum time */
   tmax = 0.;
   for (i = 0; i < nxy; i++) {
      if (tpicks[i * n1 + nps[i] - 1] > tmax)
	 tmax = tpicks[i * n1 + nps[i] - 1];
   }
   ...
   if (tslice == 0)
      vgrid = (float *) emalloc(nvs * nx * ny * sizeof(float));
