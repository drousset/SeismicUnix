...
"                 -1=only print good trace positions)     \n"
...
"                   the desired rmsout.)			\n"
"                -2=drop the trace with the bad amplitudes \n"
...
"  graph=0       create a x-window graph of amplitude histogram \n"
"                0=no 1=yes \n"
" 							         \n"
" Notes:							 \n"
...
main(int argc, char **argv)
{
	...
	int itrace;
	int nbad, ibad, report_good, action, graph;
	char *printout;
	FILE *printfp;
	float fmax, fmin;
	int itmp, i, itrtmp; 
...
/* parameters fetching */
	...
	if (!getparint("graph",&graph)) graph=0;
...
/* processing */ 
	...
	do {
		...
		itrace +=1;
		if(action!=0) {
			if(ibad==0 || action!=-2) fputtr(outfp,&trin);
		}
		rmssum += rms;
		ntrace += 1; 
