...
"Revised:               Reginald Beardsley      "__DATE__;
...
int main(int argc, char **argv)
{
   ...
   getparfloat("nval", &nval);
   getparfloat("pval", &pval);
   ...
   if (!getparint("min1", &min1)){
      min1 = 1;
   }

   if (!getparint("min2", &min2)){
      min2 = 1;
   }

   if (!getparint("min3", &min3)){
      min3 = 1;
   }

   if (!getparint("min4", &min4)){
      min4 = 1;
   }

   if (!getparint("min5", &min5)){
      min5 = 1;
   }

   if (!getparint("max1", &max1)){
      max1 = n1;
   }

   if (!getparint("max2", &max2)){
      max2 = n2;
   }

   if (!getparint("max3", &max3)){
      max3 = n3;
   }

   if (!getparint("max4", &max4)){
      max4 = n4;
   }

   if (!getparint("max5", &max5)){
      max5 = n5;
   }

   if (!getparfloat("zero", &zero)){
      zero = 0.;
   }


   ir = 0;
   if (getparfloat("rbeg", &rbeg) && getparfloat("rend", &rend)){
      ir = 1;
   }
