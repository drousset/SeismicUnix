...
"or fzt= dzt= (when ttfile is missing) \n"
"fzt=                   depth of first sample of travel time ",
"dzt=                   depth sampling interval of travel time ",
...
"tau0=from_input        time of first sample of output trace (in ms)	",
"dtau=from_input        time sampling interval of ouptut trace (in ms)	",
"tpowaf=0.0             power of time gain to be appled to the output    ",
"                       after migration ",
" ",
...
"                       if given, delrt in the trace header will be 	",
"                       kept. otherwise reset to tau0 or fzo			",
"tracekey=              key word for trace number ",
"linekey=               key word for line number ",
"	",
" ",
"AUTHOR:		Z Li,        12/94   			",
NULL};
...
main(int argc, char **argv)
{
	...
	int cdppds, cdppdds, cdppdl, cdppddl, cdpnum;
        int iix, iiy;
	float *zgain, zpow, tpowaf;
	String tracekey, linekey;
	int ikey=0;
	...
	if (!getparfloat("x1",&x1)) err("must specify x1");
		...
		if(!getparstring("hzgrid",&hzgrid)) ihz=0;
	...
	if(kzorkt==0) {
		...
		if((ttfp = fopen(ttfile,"r"))!=NULL) {
			...
			if(!getparfloat("fzt",&fzt)) err(" missing fzt ");
			if(!getparfloat("dzt",&dzt)) err(" missing dzt ");
		}
		nz0 = (int)((fzo-fzt)/dzt);
		fz = fzt+nz0*dzt;
		...
		if(noinput==0) {
			...
			if(!getparint("ntau",&ntau)) err(" ntau missing");
        	if(!getparfloat("dtau",&dtau)) err(" dtau missing");
			dtau = dtau * 0.001;
        	if(!getparfloat("tau0",&tau0)) err(" tau0 missing");
			tau0 = tau0 * 0.001;
		}
	...
	if(kzorkt==0) fprintf(stderr," nzo=%d dzo=%g fzo=%g \n",nzo,dzo,fzo);
	if(kzorkt==1) fprintf(stderr," ntau=%d dtau=%g tau0=%g \n",
		ntau,dtau*1000,tau0*1000);
	fprintf(stderr," start output ... \n");

	bh.hns = nzo;
	itmp = dzo*1000;
	if(itmp>32000) itmp = itmp/1000;
	bh.hdt = itmp;
	if(kzorkt==1) bh.hdt = dtau * 1000000;
	bh.fold = nofo;
	if(nofo>1) {
		bh.tsort = 2;
	} else {
			...
			for(io=0;io<nofo;io++) {
				for(i=0;i<ni;i++) {
					...
					if(i==0) {
						...
						if(tr.ungpow<tra.ungpow && tr.trid==1 && tra.trid==1) 
							bcopy(&tr,&tra,240);
				...
				if(flexbin==0) {
					if(kzorkt==0) {
                   		for(iz=0;iz<nz;iz++)
                           	fold[iz] = sqrt(fold0+folds[itmp1+iz]);
                           	for(iz=0;iz<nzo;iz++){
                               	zi=(fzo+iz*dzo-fz)/dzt;
                               	izi = zi;
                              	if(izi>nz-2) izi = nz-2;
                               	scale = (1.0-zi+izi)*fold[izi]+ 
									(zi-izi)*fold[izi+1];
                                tra.data[iz] = migs[itmp+iz]/scale;
                           	}
					} else {
						scale = folds[itmp1];
						if(scale>1.) {
						    for(it=0;it<ntau;it++)
                               	tra.data[it] = migs[itmp+it]/scale;
						} else {
						    for(it=0;it<ntau;it++)
                                tra.data[it] = migs[itmp+it];
						}
					}
                } else {
                    for(iz=0;iz<nzo;iz++) tra.data[iz] = migs[itmp+iz];
                }
				...
				if(kzorkt==0) {
					...
					if(ihz==0) tra.delrt = fzo;
				} else {
					if(ihz==0) tra.delrt = tau0*1000;
                   	tra.ns = ntau;
                   	tra.dt = dtau * 1000000.;
					if(tra.trid!=1) {
                    	tra.d1 = tra.dt;
