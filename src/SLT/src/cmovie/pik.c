/*====================================================================*\
   ...
   which appear to be used for annotation of the frame positions.
...
void     PikWrite(char *filename, FILE * fd)
{
...

    {
       ...
       sprintf( cmd ,"./disp.cigs.sh %d %d %d" 
        ,(int)AxisValue(DataAxis(data, 3), pik->pik[pik->npik-2][3])/10
        ,(int)AxisValue(DataAxis(data, 2), pik->pik[pik->npik-2][2])
        ,(int)AxisValue(DataAxis(data, 2), pik->pik[pik->npik-1][2]) );
