...
10	end do
	...
	return
 	...
 	subroutine transdk(nx,ny,nz,nxt,nyt,nx0,ny0,vread,vt,ivdisk)  
	integer nz,ny,nx,nxt,nyt,nx0,ny0,ivdisk
	integer i,i0,ix,iy,iz 
 	real vt(*),vread(nz)
	...
	do 10 ix=0,nxt-1
		i = iy*nxt*nz+ix*nz
		i0 = (iy+ny0)*nx+(ix+nx0)+1
		read(ivdisk,rec=i0,iostat=ios) vread
 		do 20 iz=1,nz
     		    vt(i+iz) = vread(iz)
20		end do
10	end do
