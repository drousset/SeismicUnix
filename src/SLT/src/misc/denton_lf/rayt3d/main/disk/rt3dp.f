...
cccc fortran subroutine to do 3d ray tracing in parallel
	...
	subroutine rt3dp(np,nzyx,nzyxo,nz,ny,nx,nzo,nyo,nxo,ek,nt,
     ...
     m   vv,idisk,filevxx,filevxy,filevxz,filevyy,filevyz,filevzz,
     o	 lenvxx,lenvxy,lenvxz,lenvyy,lenvyz,lenvzz)
	...
	character*(*) filevxx,filevxy,filevxz,filevyy,filevyz,filevzz
	integer idisk,lenvxx,lenvxy,lenvxz,lenvyy,lenvyz,lenvzz
	integer irecl,idvxx,idvxy,idvxz,idvyy,idvyz,idvzz
	...
	real dp121t(n1,np),dp122t(n1,np),dq111t(n1,np),dq112t(n1,np),
     ...
     3	 dq221t(n1,np),dq222t(n1,np)
	    ...
	    if (idisk.eq.1) then
			irecl = nz*4
			idvxx = 10
			open(idvxx,file=filevxx(1:lenvxx),
     1			form="unformatted",iostat=ios,
     2			status="unknown",access="direct",recl=irecl)
			idvxy = 15
			open(idvxy,file=filevxy(1:lenvxy),
     1			form="unformatted",iostat=ios,
     2			status="unknown",access="direct",recl=irecl)
			idvxz = 20	
			open(idvxz,file=filevxz(1:lenvxz),
     1			form="unformatted",iostat=ios,
     2			status="unknown",access="direct",recl=irecl)
			idvyy = 25	
			open(idvyy,file=filevyy(1:lenvyy),
     1			form="unformatted",iostat=ios,
     2			status="unknown",access="direct",recl=irecl)
			idvyz = 30	
			open(idvyz,file=filevyz(1:lenvyz),
     1			form="unformatted",iostat=ios,
     2			status="unknown",access="direct",recl=irecl)
			idvzz = 35
			open(idvzz,file=filevzz(1:lenvzz),
     1			form="unformatted",iostat=ios,
     2			status="unknown",access="direct",recl=irecl)
	   end if
...
ccc	    	if(xs.eq.fxt) then
ccc		   if(azhx.gt.PI) azhx = PI
ccc		   if(ys.eq.fyt .and. azhx.gt.0.5*PI) azhx = 0.5*PI
ccc		   if(ys.eq.eyt .and. azhn.lt.0.5*PI) azhn = 0.5*PI
ccc	    	else if(xs.eq.ext) then
ccc		   if(azhn.lt.PI) azhn = PI
ccc		   if(ys.eq.fyt .and. azhn.lt.1.5*PI) azhn=1.5*PI
ccc		   if(ys.eq.eyt .and. azhx.lt.1.5*PI) azhx = 1.5*PI
ccc	    	else if(ys.eq.eyt) then
ccc		   if(azhn.lt.0.5*PI) azhn = 0.5*PI
ccc		   if(azhx.gt.1.5*PI) azhx = 1.5*PI
ccc   	    	end if

c       reassign source locations if needed to reduce round-off errors
		if(abs(ys-fyt).lt.0.1) ysp(ip) = ysp(ip) + 0.1
		if(abs(ys-eyt).lt.0.1) ysp(ip) = ysp(ip) - 0.1
		if(abs(xs-fxt).lt.0.1) xsp(ip) = xsp(ip) + 0.1
		if(abs(xs-ext).lt.0.1) xsp(ip) = xsp(ip) - 0.1

cc avoid grid point
ccc		tmp = (ys - fyt)/dy
ccc		itmp = tmp
ccc		if(abs(tmp-itmp).lt.0.1) ysp(ip) = ysp(ip) - 1
		tmp = (xs - fxt)/dx
		itmp = tmp
		if(abs(tmp-itmp).lt.0.1) xsp(ip) = xsp(ip) - 1
		...
		if(idisk.eq.0) then
         ...
         call transdk(ny,nx,nz,nyt,nxt,ny0,nx0,vv,vxxt(1,ip),idvxx)
         call transdk(ny,nx,nz,nyt,nxt,ny0,nx0,vv,vxyt(1,ip),idvxy)
       	 call transdk(ny,nx,nz,nyt,nxt,ny0,nx0,vv,vxzt(1,ip),idvxz)
         call transdk(ny,nx,nz,nyt,nxt,ny0,nx0,vv,vyyt(1,ip),idvyy)
         call transdk(ny,nx,nz,nyt,nxt,ny0,nx0,vv,vyzt(1,ip),idvyz)
         call transdk(ny,nx,nz,nyt,nxt,ny0,nx0,vv,vzzt(1,ip),idvzz)
		end if
	   end do
