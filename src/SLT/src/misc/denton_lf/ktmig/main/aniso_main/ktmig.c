...
"velfile=               RMS velocity file name \n"
"                        (velocity is stored as v(nvs,nsv,nlv)) \n"
"\n"
"Optional parameters:							\n"
"etafile=               anisotropic pamaeter eta file name \n"
"                       (eta is stored as eta(nvs,nsv,nlv)) \n"
"etatype=0              type of eta: 0=effective; 1=instantaneous. \n"
"                       when etafile is not given, isotropic migration \n" 
"                       will be performed \n"
...
void pstmeta_(float *trace,int *nt,float *t0,float *dt,float *sx,float *sy,
	float *gx,float *gy,int *imute,
	float *fovt2,float *mig,int *nxy,int *ntm,float *x,float *y,
	float *tm,float *aperx,float *apery,float *fold,float *w1,
	float *f0,float *df,int *nf,float *ftaper,
...
main(int argc, char **argv)
{
	...
	char *velfile,*datain,*dataout,*etafile;
	int etatype=0;
	...
	float tpowaf;
	float xs, ys, xg, yg, dt, dtau, tau0, v, *vel, *eta;
	...
	if(etatype!=-1) {
		...
		etaeff = (float*) emalloc(ntv*sizeof(float));
		eta = (float*) emalloc(ntv*sizeof(float));
	}
	...
	if(ivdisk==0 && etatype!=-1) 
		etas = (float*) emalloc(ntv*nsv*nlv*sizeof(float));
	...
	if(ivdisk==0 && etatype!=-1) {
		fseek2g(etafp,0,0);
		efread(etas,sizeof(float),ntv*nsv*nlv,etafp);
	}
...
*/
		...
		if(cdpnow!=cdppre || vread==0 ) { 
			...
			if(ivdisk==1) {
				...
				if(etatype!=-1) {
					fseek2g(etafp,lpos,0); 
					efread(eta,sizeof(float),ntv,etafp);
				}
			} else {
				for(it=0;it<ntv;it++)
				...
				if(etatype!=-1) {
					for(it=0;it<ntv;it++)
						eta[it] = etas[it+(isv+ilv*nsv)*ntv];
				}
			}
			vread = 1;
		}
		...
		/* compute eta at output travel time */
		if(etatype==0) {
			for(it=0;it<ntau;it++) {
				tmp = (tmig[it]-t0v)/dtv;
				itv = tmp;
				if(itv<0) {
					...
					tmp = tmp - itv;
					v = (1.-tmp)*eta[itv]+tmp*eta[itv+1];
				}
				etamig[ktrace*ntau+it] = v;
			}
		} else if(etatype==1) {
			tmp = 0.;
			etaeff[0] = eta[0];
			for(it=1;it<ntv;it++) {
				vr4 = vel[it]*vel[it]*vel[it]*vel[it];
				tmp = tmp + vr4*(1.+8.*eta[it])*dtv;
				etaeff[it] = 0.125*(tmp/(it*dtv+t0v)/vr4-1.);
			}
			for(it=0;it<ntau;it++) {
				tmp = (tmig[it]-t0v)/dtv;
				itv = tmp;
				if(itv<0) {
					...
					tmp = tmp - itv;
					v = (1.-tmp)*etaeff[itv]+tmp*etaeff[itv+1];
				}
				etamig[ktrace*ntau+it] = v;
			}
		}
		...
		if(ktrace==mtrace || ktrace==ntras) {
			migs(nlout,nsout,nofo,mig,ntau,
				...
				resamp,ires,ntres,
				ss2,scs,scg,ncpu,
				vr2,vi2,vq4,tau,etamig,etatype);
...
void migs(int nl, int ns, int nofo, float *mig, int ntau,
	...
	for(itr=0;itr<ktrace;itr++) {
		...
		if(etatype==-1) {
			...
			pstmeta_(trace,&ntl,&tminl,&dtl,&ss[itr],&sl[itr],
			&gs[itr],&gl[itr],&imutel[itr],
			fovt2+itr*ntau,mig+ii*ntau*nsl,&nsl,&ntau,s,l,
			tm,&apers,&aperl,fold+nsl*ii,w1,
			&f0,&df,&nf,&ftaper,
			...
			&ksmax,&klmax,&dtau,&tau0,&angmax,
			indxw,&nindxw,&incdxw,&ncpu,
			resamp,&ires,&ntres,s2,scs,scg,etamig+itr*ntau);
		}
	}
