...
2000                		continue
			    	gap = ixn-ix0+1
cc		write(*,*) 'gap=',gap,' maxgap=',maxgap
cc		write(*,*) 'x=',x,' a0=',a(0),' a(n-1)=',a(n-1)
cc		call msgsci("gag=",gap)
cc		call msgsci("maxgag=",maxgap)
cc		call msgscf("x=",x)
cc		call msgscf("a(0)=",a(0))
cc		call msgscf("a(n-1)=",a(n-1))
			    	if(gap.lt.0) gap = - gap
...
c			          		ilive(ix,iy) = -1
c			       		end if
c			    	else if(x.gt.a(n-1)) then
c			       		if (ib.eq.1) then
c			          		ti(ix,iy) = b(n-1)
c			          		ilive(ix,iy) = -1
c			       		end if
c		        	else if(x.gt.a(0).and.x.lt.a(n-1)) then
c			       		call lin1d(a(0),b(0),n,x,tmp,1,itmp)
c			       		ti(ix,iy) = tmp
c			       		ilive(ix,iy) = -1
c			    	end if
		        	if(x.gt.a(0).and.x.lt.a(n-1)) then
...
5000                		continue
		        	...
		        	if(y.gt.a(0).and.y.lt.a(n-1)) then
			    		call lin1d(a(0),b(0),n,y,tmp,1,itmp)
c			    	if (ib.eq.0) then
c                    			if (x.lt.xmin(iy) .or. 
c     1                    	   	x.gt.xmax(iy)) tmp = bv
c			    	end if
			    		ti(ix,iy) = tmp
			       		ilive(ix,iy) = 1
				end if
