...
"             =-2 convert floating point file (without gridheader) \n"
...
main(int argc, char **argv)
{
	...
	int segy;
	char c4i[4], c4o[4], c2i[2], c2o[2], c1i[1], c1o[1];
	char *cbuf, *cout;
	...
	if(segy==1) {
		...
		n2 = usghin.n2;
		if(n2==0) n2=1;
		if(usghin.n3>1) n2 = n2 * usghin.n3;
		if(usghin.n4>1) n2 = n2 * usghin.n4;
		if(usghin.n5>1) n2 = n2 * usghin.n5;
		dtype = usghin.dtype;
		fseek64(infp,0,0);
		...
		for(i1=0;i1<n1;i1++) {
			...
			if(dtype==1) {
				fread(c1o,dtype,1,infp);
				fwrite(c1o,dtype,1,outfp);
			} else if(dtype==2) {
				fread(c2i,1,dtype,infp);
				bswap2(c2i,c2o);
				fwrite(c2o,1,dtype,outfp);
			} else if(dtype==4) {
				fread(c4i,1,dtype,infp);
				bswap4(c4i,c4o);
				fwrite(c4o,1,dtype,outfp);
			}

		}
		}
		/* last 100 bytes of header */
		do {
			if(iread==4) {
				bswap4(c4i,c4o);
