...
main(int argc, char **argv)
{
    ...
    int ic, jc, icmax; 
    char read1[16], read2[7], read3[7], read4[7];
    float *r1, *r2, *r3, *r4;
    string mutefile;
    ...
    if (!getparint("findmute",&findmute)) findmute=0;
...
*/
	      ...
	      for(i=0;i<3;i++) {
			...
			if(strncmp(read4, "      ",6)==0 || strncmp(read4, "\n",1)==0) {
				i4 = tmax;
		  	}
	     /*
...
*/
	  ...
	  /* sort the r1 into ascending order */
	  fsort = (float*) emalloc(nchange*sizeof(float));
	  isort = (int*) emalloc(nchange*sizeof(int));
	  ipairs = (int*) emalloc(nchange*sizeof(int));
	  rr = (float*) emalloc(nchange*n1*sizeof(float));
	  ...
	  qkisort(nchange,fsort,isort);
	  for(i1=0;i1<nchange;i1++) fsort[i1] = r1[i1];
	  for(i1=0;i1<nchange;i1++) r1[i1] = fsort[isort[i1]];
	  for(i1=0;i1<nchange;i1++) ipairs[i1] = npairs[i1];
	  for(i1=0;i1<nchange;i1++) npairs[i1] = ipairs[isort[i1]];
	  for(i1=0;i1<nchange;i1++)
		for(i2=0;i2<n1;i2++)
			rr[i1*n1+i2] = r2[i1*n1+i2];
	  for(i1=0;i1<nchange;i1++)
		for(i2=0;i2<n1;i2++)
			r2[i1*n1+i2] = rr[isort[i1]*n1+i2];
	  for(i1=0;i1<nchange;i1++)
		for(i2=0;i2<n1;i2++)
			rr[i1*n1+i2] = r3[i1*n1+i2];
	  for(i1=0;i1<nchange;i1++)
		for(i2=0;i2<n1;i2++)
			r3[i1*n1+i2] = rr[isort[i1]*n1+i2];
	  free(isort);
	  free(ipairs);
	  free(rr);
	  free(fsort);
...
*/
		...
		if(itoff==1) {
			ts = (float*) malloc(nt*sizeof(float));
			offts = (float*) malloc(nt*nchange*sizeof(float));
			offt = (float*) malloc(nt*sizeof(float));
			indx = (int*) malloc(nt*sizeof(float));

			for(i1=0;i1<nt;i1++) {
				ts[i1] = ot + i1*dt;
			}
			for(i2=0;i2<nchange;i2++) {
	    		n = npairs[i2];

				/*
				for(i1=0;i1<n;i1++)
				fprintf(stderr," t=%g off=%g \n",r2[i1+i2*n1],r3[i1+i2*n1]);
				*/	

				lin1d_(r2+i2*n1,r3+i2*n1,&n,ts,offts+i2*nt,&nt,indx);

				/*
				for(i1=0;i1<nt;i1=i1+10)
					fprintf(stderr,"i1=%d ts=%g offts=%g \n",
...
/*
			fprintf(stderr,"i11=%d i12=%d res1=%g \n",i11,i12,res1);
*/
			for(i1=0;i1<nt;i1++) {
				offt[i1] = offts[i11*nt+i1] + 
					 res1*(offts[i12*nt+i1] - offts[i11*nt+i1]);
					 /*
					fprintf(stderr,"i1=%d offt=%g \n",i1,offt[i1]);
					*/
			}
			for(it=0;it<nt;it++) {
				if(offt[it]<tr.offset) tr.data[it] = 0.;
			}
		...
		if(findmute==1) {
			itmute_top = (int) (tmax+0.5);
			itmute_bot = (int) (tmax+0.5);
			for(it=0;it<nt;it++) {
				if(tr.data[it]!=0.) {
					itmute_top = ot + it*dt;
					itmute_bot = tmax;
				...
				/*
	   		fprintf(stderr,"itmute_top=%d itmute_bot=%d cdpt=%d \n", 
			   itmute_top, itmute_bot,tr.cdpt);
			   */
		} else {
	...
	tr.muts = 0;
	tr.mute = itmute_top;
	if(itmute_bot<tmax) tr.mutb = itmute_bot;
...
*/
	...
	for(it=imute+1;it<nt;it++) tr.data[it] = 0.;
	/* tapering */
	if( mutetaper > 0 ) {
