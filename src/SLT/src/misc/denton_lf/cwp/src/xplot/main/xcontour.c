...
/* XCONTOUR: $Revision: 1.4 $ ; $Date: 1997/04/16 18:09:50 $	*/
...
{
	int n1,n2,nc,n1tic,n2tic,nfloats,
...
/*			XClearWindow(dpy,win);*/
		...
		/* else if window exposed */
			  ...
			  newContour(dpy, win,gci,
				     x,y,nxb,&ixX[0],nyb,
				     &iyX[0],czb,
				     nc,c,cwidth,ccolor,cdash,labelcf,labelcper,nlabelc,labelcfont,labelccolor);
			free1float(ixX);
			free1float(iyX);
			}
...
static void newContour (Display *dpy, Window win,GC gci,
	...
	int nc, float c[],float cwidth[], char *ccolor[],float cdash[],int labelcf,int labelcper,int nlabelc,char *labelcfont,char *labelccolor)
{
        float *w;
	int ic,linewidth=0;
	...
	/*	(note that small line segments will mask the dash settings) */
	  ...
	  xContour(dpy,win,gcc,gcl,&c[ic],nx,x,ny,y,z,lcflag,labelcfont,labelccolor,w);
	  }
	free1float(w);
}
	...
	float p2beg, float p2end)
{
	...
	XDrawString(dpy,win,gc,NINT(xoffset),NINT(yoffset+overall.ascent),
