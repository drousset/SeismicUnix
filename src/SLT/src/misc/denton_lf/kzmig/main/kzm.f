...
c$par doall private(itr,yi,xi,dr2,tmp,tmp1,iang,iz0)
c$par doall private(izo0,diss,cosas,sinas,disg,cosag,sinag)
c$par doall private(ar,jr,srs,srs0,jrs,srg,srg0,jrg,iz,izo)
c$par doall private(tsd,tgd,psd,pgd,px,py)
c$par doall private(slopex,slopey,fxmax,fymax,zi,izi,sz,omsz)
c$par doall private(ti,iw,amp,iti,res,zi0)
	do itr=1,nxy
 	   	yi = y(itr)
 	    xi = x(itr)
...
c 	apply the sqrt(-iw) filter
		do k=2,nfft/2
	   		...
	   		tr = work1(ir)*sqrtf(k)
	   		ti = work1(ii)*sqrtf(k)
cccccc 	apply the sqrt(-iw) filter
cc 	   		work1(ir) = tr+ti 	
cc	   		work1(ii) = ti-tr 	
cccccc 	apply the sqrt(iw) filter
 	   		work1(ir) = tr-ti 	
