...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      SUBROUTINE RECCON(X,z,GEOINC,geoz,PI,BETNEW,NOCONV,s0,
...
c     if so, check to see if it has found a solution.
      ...
      I = 0
      CALL NEWTON(X,z,RESID,FAIL,.FALSE.,0.,0.,
     1          XINT,ZINT,A0,A1,A2,A3,SIGN,NPTS,NINT,NORDER,
     2          DZ,DDZ,DELTAX,DELTAZ,D,V,N,
     3          MAXINT,MAXSPL,MAXN,MAXNP1,MXSPM1,
...
*---------------------------------------------------------------------
      ...
      SUBROUTINE NEWTON(X,z,RESID,FAIL,head,sgnoff,sinthc,
     1          XINT,ZINT,A0,A1,A2,A3,SIGN,NPTS,NINT,NORDER,
     2          DZ,DDZ,DELTAX,DELTAZ,D,V,N,
     3          MAXINT,MAXSPL,MAXN,MAXNP1,MXSPM1,
     4		SUBDJ,DJ,SUPDJ,PHI)
...
c     Evaluate the diagonal.
      DO 100  K = 1,  kmax
         kp1 = k + 1
c	 write(*,*) "v(kp1)=",v(kp1), " v(k)=",v(k)," k=",k
...
5        IF(X(I).GT.XINT(L,J)) THEN
         ...
         J = J - 1
	 T = X(I)
         Z(I)   = A0(L,J) + A1(L,J) * T + A2(L,J) * T**2
     $          + A3(L,J) * T**3

         DZ(I)  = A1(L,J) + 2.* A2(L,J) * T + 3.*A3(L,J) * T**2
         DDZ(I) = 2.* A2(L,J) + 6.*A3(L,J) * T
