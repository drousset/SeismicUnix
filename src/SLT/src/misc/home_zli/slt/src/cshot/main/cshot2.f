...
c     resamp should be an EVEN integer OR 1 for no resampling
     ...
     :           irout,    irout1,  iroutn,  idummy
      ...
      CHARACTER   INFIL*40,   OUTFIL*40,   job*1

      logical     shtsrt,  recsrt,   caustc,   head,
     :            eof,     offsrt

      INTEGER        ret, getparstring
      CHARACTER * 80 PARAM2
     ...
     :            FUDGE = 1.)
      ...
      ret = getparstring('param2',PARAM2) 
      IF( ret .eq. 0 ) THEN 
        WRITE(stderr,'(1x,A)') 'Can''t get parameter param2.'
        STOP
      ENDIF
C
      ...
      GO TO 10
5     WRITE(STDERR,'(A,1X,A)') 'Can''t open file : ', PARAM2
      STOP
