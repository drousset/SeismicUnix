...
"dzo=0.1*dzo            Output migrated depth sample interval  		",
 "                       						",
...
"offmax=13000           upper limit of offset (default in ft)		", 
...
"v0=5000(ft/s)         	reference velocity at the surface.   		",	
"dvz=0.0                reference velocity gradient in depth direction. ",
...
"                        if isave=1) 					",
"tapecntl=0             Input tape control (for tape or pipe input)	",
"                       0=tape mounted from begining VSN tpae of a DSN	",
"                         The program will read and not process until 	",
"                         the input trace counter exceeds the number of ",
"                         traces process in the previous run.		",
"                         This option should be used when input is PIPED",
"                         to kzmig					",
"                       1=tape mounted from 1st tape to be processed in	",
"                         the current run. The program will set the 	",
"                         input trace counter to the number of traces 	",
"                         processed in the last run and start to process",
"                         traces right away. Used when input are more   ",
"                         than one disk files (no pipe).		",
"                      -1=input dataset is from disk (not tape and not	",
"                         pipe). The program will them perform a seek	",
"                         to the proper trace position, before reading	",
"                         and processing the data. Use only when a 	",
"                         single disk file is going to be used for 	",
"                         migration.            			",
...
"                       has been migrated in total upto the last run	",
...
"                       filters						",
"nf=6                   number of anti-aliasing filters			",
...
" ",
"Notes:		",
"I/O managements are mostly copied from ktmig.c by Zhiming Li.	",	
...
main(int argc, char **argv)
{
	...
	float xs, ys, xg, yg, dt, dzo, fzo, v0, dvz; 
	float *ss, *sl, *gs, *gl, ms, ml, ofomin, dofo; 
	float *st, *lt;
	char *buf;
	int itmp, itmp2, ntl;
	...
	int range,gath,oldheadx,oldheady,newheadx,newheady,gottrace,ready,done,
	    nr,nseek,ntab,itab,nxs0,nys0,nxs,nys,iz,ixs,iys,izi,izw,
	...
	envs = (char*) emalloc(80*sizeof(char));
        if(!getenv("MP_SET_NUMTHREADS")) {
                sprintf(envs,"%s=%d","MP_SET_NUMTHREADS",ncpu);
                putenv(envs);
        }
	...
	if(!getparint("izw",&izw)) izw = (fzt<4*dzt)? 4-fzt/dzt: 0;
    	...
    	if (!getparfloat("dvz",&dvz)) dvz = 0.;
     	...
     	if (!getparfloat("offmax",&offmax)) offmax = 13000;
...
/*	check migration output range	*/
	...
	if(ntrpre>0) {
		...
		efread(mig[0],sizeof(float),ltmp,imgfp);
		efclose(imgfp);
		...
		fseek2g(fldfp,0,1);
		efread(fold[0],sizeof(float),nofo*nsout*nlout*nz,fldfp);
		efclose(fldfp);
		...
		efwrite(mig[0],sizeof(float),nofo*nsout*nlout*nzo,imgfp);
		...
		fseek2g(fldfp,0,1);
		bzero(fold[0],nofo*nsout*nlout*nz*sizeof(float));
		efwrite(fold[0],sizeof(float),nofo*nsout*nlout*nz,fldfp);
		efclose(fldfp);
	...
	fprintf(jpfp," ntrpre=%d nt=%d ntl=%d dt=%g t0=%g nfft=%d\n",
 	...
 	fprintf(jpfp," tpow=%g zpow=%g ofomin=%g dofo=%g nofo=%d isave=%d \n",
		tpow,zpow,ofomin,dofo,nofo,isave);
...
*/
		...
		if(ndskwt>=ntrdsk && ntrdsk>0) {
                /* update disk file */
                        fldfp = efopen(diskfld,"w");
                        fseek2g(fldfp,0,1);
                        efwrite(fold[0],sizeof(float),
				nz*nlout*nsout*nofo,fldfp);
                        efclose(fldfp);
                        imgfp = efopen(diskimg,"w");
                        fseek2g(imgfp,0,1);
	...
	if(isave==1 && ntrace>0) {
		fldfp = efopen(diskfld,"w");
		fseek2g(fldfp,0,1);
		efwrite(fold[0],sizeof(float),nlout*nsout*nz*nofo,fldfp);
		efclose(fldfp);
	} 
	...
	int nr, float dr, float *tb, float *pb, float **ttab, int gath) 
{
	...
	tit = alloc1float(nz+1);
	wt = alloc1float(nz+1);
	ampt = alloc1float(nz+1);
/*
...
*/
	   ...
	   /* linearly inpterpolate traveltimes to source and receiver */
