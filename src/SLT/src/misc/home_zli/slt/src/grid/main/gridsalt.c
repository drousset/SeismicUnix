...
main(int argc, char **argv)
{
	...
	&&  getparstring("vbot2grid",&vbot2grid) ) {
				...
				if(saltvelo!=-9898) {
					...
					for(i1=i1top;i1<i1bot2;i1++) {
						if(i1>=0 && i1<=n1-1) {
							...
							tmp = tmp - top;
							if(bot2>top && tmp>0.) {
								grid[i1] = vtop[i3*n2+i2] + tmp * 
								(vbot2[i3*n2+i2]-vtop[i3*n2+i2]) /
								(bot2-top);
							} else {
								grid[i1] = vtop[i3*n2+i2];
							}
							if(grid[i1]<0.) 
		fprintf(stderr,"top=%g bot2=%g tmp=%g \n",
							top,bot2,tmp);
						}
					}
				}
			...
			/* salt below top */
				...
				if(i1bot2>i1bot) i1bot2=i1bot;
				for(i1=i1top;i1<i1bot2;i1++) {
					if(i1<n1 && i1>=0) grid[i1] = saltvelo;
				}
				if(i1bot2<i1top) i1bot2 = n1;
			/* top to bottom salt */
			} else if(top2<=top &&  top<= bot) {
