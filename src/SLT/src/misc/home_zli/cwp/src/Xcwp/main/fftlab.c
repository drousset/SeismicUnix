...
static void originChangeCB (Widget w, caddr_t client_data, caddr_t call_data);
static void zeroAllCB (Widget w, caddr_t client_data, caddr_t call_data);
static void quitCB (Widget w, caddr_t client_data, caddr_t call_data);
static void editDone (Samples *s);
static void newData (Samples *s);
static void zeroData (Samples *s);
static void transform (CSamples *this, CSamples *that);
...
static void CreateApplication (Widget parent)
{
	...
	XtManageChild(formo);
	framee = XcwpCreateStringRadioButtons(formo,"Editing",
		nEditingButtons,editingButtons,0,
		editingCB,NULL);
	XtManageChild(framee);
	framel = XcwpCreateStringRadioButtons(formo,"Length",
	...
	XtAddCallback(toggleb,XmNvalueChangedCallback,originChangeCB,NULL);
	n = 0;
	XtSetArg(args[n],XmNlabelType,XmSTRING); n++;
	...
	XtManageChild(pushb);
	XtAddCallback(pushb,XmNactivateCallback,zeroAllCB,NULL);
	n = 0;
	XtSetArg(args[n],XmNlabelType,XmSTRING); n++;
	...
	pushb = XmCreatePushButtonGadget(formo,"",args,n);
	XtManageChild(pushb);
	XtAddCallback(pushb,XmNactivateCallback,quitCB,NULL);
...
{
	/* set edit mode */
	samplesSetEditMode(fReal,mode);
	samplesSetEditMode(fImag,mode);
	samplesSetEditMode(gReal,mode);
	samplesSetEditMode(gImag,mode);
}
...
static void editDone (Samples *s)
{
	...
	/* determine this and that */
	this = (fReal==s || fImag==s) ? fComplex : gComplex;
	that = (fReal==s || fImag==s) ? gComplex : fComplex;
	
	/* transform this to that and draw that */
	transform(this,that);
	pv = determinePlotValue(that);
	samplesSetPlotValue(that->r,pv);
	samplesSetPlotValue(that->i,pv);
	samplesDraw(that->r);
	samplesDraw(that->i);
}
...
static void transform (CSamples *this, CSamples *that)
{
	...
	/* merge real and imaginary parts from this */
	thisr = this->r->data;
	thisi = this->i->data;
	...
	/* unmerge real and imaginary parts from that */ 
	thatr = that->r->data;
	thati = that->i->data;
	for (i=0; i<nSamples; i++) {
		thatr[i] = c[i].r;
