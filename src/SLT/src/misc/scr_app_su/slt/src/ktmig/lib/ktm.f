...
cccc$par doall private(ixy,yi,xi,dys,dyg,dy2s,dy2g,dxs,dxg,dx2s)
cccc$par doall private(dx2g,d2s,d2g,xp,yp,tmp,t,it0,dov2,iang,iiend)
cccc$par doall private(ii,it,coss,ots,cosg,otg,fxmax,iwx)
cccc$par doall private(fymax,iwy,iw,itmp,itmp2,sc2,sc1,a)
cccc$par doall private(ts,tg)
		do ixy=1,nxy
	   		yi = y(ixy)
	        xi = x(ixy)
...
ccccc cos(as)+cos(ag)
		       ...
		       do it=it0,ntm
                  ...
                  ts=sqrt(1.+fovt2(it)*scs(it,ixy))
                  tg=sqrt(1.+fovt2(it)*scg(it,ixy))
