...
c       fzo(nxy)    first output depth sample  
...
c
	...
	real x(nxy),y(nxy),fold(nz,nxy),fzo(nxy)
	integer imute, nxy
	real mig(nzo,nxy)
...
cccc$ call schedctl(SCHEDMODE, SGS_FREE, 0)

cccc migration over output traces
cccc
ccccc$dir force_parallel
ccccc$doacross local(itr,yi,xi)
ccccc$& local(dr2,tmp,tmp1,iang,iz0,izo0,diss,cosas,sinas,disg,cosag,sinag)
ccccc$& local(ar,jr,srs,srs0,jrs,srg,srg0,jrg,iz,izo,tsd,tgd,psd,pgd,px,py)
ccccc$& local(slopex,slopey,fxmax,fymax,zi,izi,sz,omsz,ti,iw,amp,iti,res,zi0)
ccccc$& shared(nxy,x,y,xm,ym,aperxx,aperyy,otan2,fz,odz,fzo,odzo,dr,nr,nz)
ccccc$& shared(tb,ts,tg,odt,shift,dt,nt,imute,fold,kxmaxo,kymaxo,rdz)
ccccc$& shared(pb,sx,gx,sy,gy,epsx,epsy,nf,f0o,nzo,mig,tracef,work1)
ccccc$& shared(tit,ampt,wt)
ccccc$& mp_schedtype=dynamic
ccccc$& chunk=1

cccc$par doall private(itr,yi,xi,dr2,tmp,tmp1,iang,iz0)
cccc$par doall private(izo0,diss,cosas,sinas,disg,cosag,sinag)
cccc$par doall private(ar,jr,srs,srs0,jrs,srg,srg0,jrg,iz,izo)
cccc$par doall private(tsd,tgd,psd,pgd,px,py)
cccc$par doall private(slopex,slopey,fxmax,fymax,zi,izi,sz,omsz)
cccc$par doall private(ti,iw,amp,iti,res,zi0)
	do itr=1,nxy
 	   	yi = y(itr)
 	    xi = x(itr)
		zi0 = 1+(fzo(itr)-fz)*odz-rdz
...
ccccc 			if(tit(iz,itr) .le. 2*nt ) then
     ...
     1                	+sz*ampt(izi+1,itr)
c			  if(tit(izi,itr).ge.nt .or.
c     1				tit(izi+1,itr).ge.nt) ti = imute
			  		iti = ti  
			  		res = ti - iti
			  		iti = max0(iti,imute)
			  		iti = min0(iti,nt)
