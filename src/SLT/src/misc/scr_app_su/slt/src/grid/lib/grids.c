...
*/
	...
	long long isize, jsize, iseek, jseek;
	off_t lofset;
	...
	*/
		...
		iseek = 0;
		bcopy(&iseek,&lofset,8);
		fseeko(gfp,lofset,1);
		lofset = ftell2g(gfp);
		bcopy(&lofset,&iseek,8);
		/* check grid file size */ 
		jsize = 0;
		bcopy(&jsize,&lofset,8);
		fseek2g(gfp,lofset,2);
		lofset = ftell2g(gfp);
		bcopy(&lofset,&jsize,8);
...
*/
	...
	if(ftype==DISK) {
		...
		isize = isize*n4*n5*idtype + GHDRBYTES;
			...
			isize = isize - GHDRBYTES;
			bcopy(&isize,&lofset,8);
			fseek2g(gfp,lofset,0);
		} else {
                	ierr = 1;
        		warn(
