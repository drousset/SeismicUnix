...
"ncpu=1                 number of CPUs to use (0<= ncpu <=32)		\n"
...
main(int argc, char **argv)
{
	...
	lwork = lwork *sizeof(complex);
	lmem = nx*ny;
	lmem = lmem * (2+nv+nq);
	tmp = ( (naux1+naux2+lvec)*ncpu+ 2*lplane+nf+lmem+ncpu );
	tmp = tmp * sizeof(float);
	lwork = lwork + tmp;
	tmp = iisave*2*lplane*ncpu*sizeof(complex);
	lwork = lwork + tmp;
