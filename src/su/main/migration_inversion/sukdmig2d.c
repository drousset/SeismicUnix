...
void resit(int nx, float fx, float dx, int nz, int nr, float dr,
	float **tb, float ***ttab, int ns, float ds, float fs);
...
main(int argc, char **argv)
{
	...
	/* read file to compute traveltime residual	*/
	for (is = 0; is < ns; ++is){
	...
	fprintf(jpfp, " compute traveltime residual .");
	timeb(nr, nzt, dxo, dzt, fzt, dvz, v0, tb, pb, cs0b, angb);
	fprintf(jpfp, ".. \n");
	resit(nxt, fxt, dxt, nzt, nr, dxo, tb, ttab, ns, ds, fs);
	...
	do {
		...
		if (MIN(sx, gx) >= fs && MAX(sx, gx) <= es && MAX(gx - sx, sx - gx) <= offmax){
			/*     migrate this trace	*/
			as = (sx - fs) / ds;
			is = (int)as;
			if (is == ns - 1) is = ns - 2;
	...
	*****************************************************************************/
{
	...
	float ** tmt, ** ampt, ** ampti, ** ampt1 = NULL, *trf, *zpt;
	float amp1_iz, amp1_iz1, temp_npv;
	float tzt_iz, tzt_iz1, amp_iz, amp_iz1, ampi_iz, ampi_iz1, tm_iz, tm_iz1;
	int nzi = nz; //floats do not vectorize in for loops
	...
	for (ix = nxtf; ix <= nxte; ++ix){
		...
		for (ix = nxf; ix <= nxe; ix++)
		{
			...
			for (iz = iz0; iz < nzi; ++iz)
			{
				...
				tzt_iz = ax0*tsum[jx][jz] + ax*tsum[jx + 1][jz]
					+ srs0*tb[jrs][jz] + srs*tb[jrs + 1][jz]
					+ srg0*tb[jrg][jz] + srg*tb[jrg + 1][jz];
				tzt_iz1 = ax0*tsum[jx][jz + 1] + ax*tsum[jx + 1][jz + 1]
					+ srs0*tb[jrs][jz + 1] + srs*tb[jrs + 1][jz + 1]
					+ srg0*tb[jrg][jz + 1] + srg*tb[jrg + 1][jz + 1];

				amp_iz = ax0*ampt[jx][jz] + ax*ampt[jx + 1][jz];
				amp_iz1 = ax0*ampt[jx][jz + 1] + ax*ampt[jx + 1][jz + 1];
				ampi_iz = ax0*ampti[jx][jz] + ax*ampti[jx + 1][jz];
				ampi_iz1 = ax0*ampti[jx][jz + 1] + ax*ampti[jx + 1][jz + 1];
				tm_iz = ax0*tmt[jx][jz] + ax*tmt[jx + 1][jz];
				tm_iz1 = ax0*tmt[jx][jz + 1] + ax*tmt[jx + 1][jz + 1];

				if (npv)
				{
					amp1_iz = ax0*ampt1[jx][jz] + ax*ampt1[jx + 1][jz];
					amp1_iz1 = ax0*ampt1[jx][jz + 1] + ax*ampt1[jx + 1][jz + 1];
				}

				td = sz0*tzt_iz + sz* tzt_iz1;
				at = (iz < nzp) ? (td - ft)*odt + mtmax : (td - ft)*odt;
				jt = (int)at;
				res = at - jt;
				res0 = 1.0 - res;
				...
				temp = 0;
				temp_npv = 0;
				if (iz<nzp && jt > mtmax && jt < nt + mtmax - 1){
					// interpolate along depth if operater aliasing
					ampd = sz0*ampi_iz + sz*ampi_iz1;
					mt = (int)(0.5 + sz0*tm_iz + sz*tm_iz1);
					...
					if (npv)
						temp_npv = temp *(sz0*amp1_iz + sz*amp1_iz1);
				}
				else if (iz >= nzp && jt > 0 && jt < nt - 1){
					// interpolate along depth if not operater aliasing 
					ampd = sz0*amp_iz + sz*amp_iz1;
