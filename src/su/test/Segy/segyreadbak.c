 ...
 * Notes:
 *	Brian's subroutine, ibm_to_float, to convert IBM floating
 *	point to IEEE floating point is NOT portable and must be
 *	altered for non-IEEE machines.  See the subroutine notes below.
 *
 *	The subroutine tape_to_su is only a stub and is not portable.
 *	On machines where shorts are not 2 bytes and/or longs are not
 *	4 bytes, routines to convert SEGY 16 bit and 32 bit integers will
 *	be required.
 *
...
main(int argc, char **argv)
{
	...
	nsflag = false;
	itr = 0;
	if (buff) { /* buff test outside for efficiency--duplicate code!! */
		while (eread(tapefd, (char *) &tapetr, nsegy) && itr < trmax) {
		...
		while (efread((char *) &tapetr, 1, nsegy, tapefp) &&
							itr < trmax) {
