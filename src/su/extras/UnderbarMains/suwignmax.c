/* SUWIGNMAX: $Revision: 2.24 $ ; $Date: 89/12/01 16:39:02 $		*/
...
string sdoc = "\
									\n\
SUWIGNMAX - wiggle plot of su data traces above a graph of 		\n\
	    trace-by-trace maximum amplitude (optional running average)	\n\
									\n\
suwignmax <data [optional parameters] | tube				\n\
									\n\
Optional Parameters:							\n\
...
static string progid =
	"   $Source: /src/su/src/RCS/suwignmax.c,v $";
static string revid =
	"   $Revision: 2.24 $ ; $Date: 89/12/01 16:39:02 $";
...
int argc; char **argv;
{
	...
	void subplotw();	/* isolate vplot commands		*/
	...
	n = nt*ntr;
	maxmgv_(data, ONE, &gmax1, &displ, &n);
	...
	maxmgv_(data, ONE, &gmax2, &displ, &n);
	...
	n = nt;	/* trace is nt long, so tell maxmgv to look at nt samples */
	for (itr = 0; itr < ntr ; itr++) { 
		bcopy(data + itr*nt, tmpvec1, ntsize); 
		maxmgv_(tmpvec1, ONE, &absmax, &displ, &n);
		trmaxs[itr] = absmax; 
	} 
