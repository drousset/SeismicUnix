...
main(int argc, char **argv)
{
   ...
   if (orient == 1) {
      if (idisk == 0)
         efread(vs, sizeof(float), n1 * nx * ny, vgfp);
   } else {
      for (i1 = 0; i1 < n1; i1++) {
   ...
   if (idisk == 0)
      fprintf(stderr, " velocity function read \n");
         ...
         if (torz == 0) {
            if (vgtype == 1) {
               if (vtorz == 0) {
                  ...
                  for (i1 = 1; i1 < n1; i1++) {
                     if( v[i1] != v[i1-1] ){
                        tv[i1] = tv[i1 - 1] + log(v[i1]/v[i1-1])*d1 / (v[i1]-v[i1-1]) * 2000.;
                     }else{
                        tv[i1] = tv[i1-1] + d1/v[i1]*2000.0;
                     }
                  }
               }
            } else {
            ...
            dzdtn =
                  (zv[n1 - 1] - zv[n1 - 2]) / (tv[n1 - 1] - tv[n1 - 2]);
            ...
            dtdzn =
                  (tv[n1 - 1] - tv[n1 - 2]) / (zv[n1 - 1] - zv[n1 - 2]);
