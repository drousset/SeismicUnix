...
char *sdoc = 
"HVELADDXY - add x and y to DISCO cdplbl HANDVEL cards  	\n"
"\n"
"hveladdxy [parameters] <handvel-cdplbl >handvel-cdplbl-xy 		\n" 
"\n"
"Required parameters:						 	\n"
		...
		&& cbuf[4]=='V' && cbuf[5]=='E' && cbuf[6]=='L' )  {
			...
			if(icdp==0) err("cdp number can not be zero");
	  		...
	  		if (icdp != icdpnow) {
				...
				cdplbl = icdpnow;
				cdplbls = cdplbl / 10000;
				cdplblx = cdplbl - cdplbls*10000;
				ttrace = cdplblx;
				lline = cdplbls;

				sl2xydb(ttrace1,lline1,xx1,yy1,ttrace2,lline2,xx2,yy2,
					  ttrace3,lline3,xx3,yy3,ttrace,lline,&xnow,&ynow);

				x = xnow;
				y = ynow;

				fprintf(stderr,
"HANDVEL x y added at cdplbl=%d trace=%d line=%d x=%g y=%g for %d t-v pairs \n",
		cdplbl, cdplblx, cdplbls, x, y, nvt); 
				printhvelxy(cdplbl,x,y,nvt,times,vrms,outfp);
