...
void     migs(int nl, int ns, int nofo, float *mig, int ntau,
              FILE * imgfp, int ktrace, int ntl, float dldt,
              float *trace, float *tras, float dtl, float tminl, int nt,
              float *ss, float *sl, float *gs, float *gl, int *imute,
              float *fovt2, float *s, float *l, float *tm, int *iofs,
              float apers, float aperl, float *fold, float *w1,
              float *work1, float *work2, float *wsave, float *tracef,
              int *ifcut, int *ltaper, float ksmax, float klmax,
              float f0, float df, int nf, float ftaper, int nsave,
              int nfft, float dtau, float tau0, float angmax,
              int *indxw, int nindxw, int incdxw, float *resamp,
              int ires, int ntres, float *ss2, float *scs, float *scg,
              int ncpu, float *vr2, float *vi2, float *vq4, float *tau);

void     fhigint_(float *f0, float *df, int *nf, float *ftaper,
                  float *dt, int *ifcut, int *ltaper, float *wsave,
                  int *nsave, int *nfft);
...
void     vt2s2_(float *vr2, float *vq4, float *s2, float *tau,

                int *ntau);

void     pstm3d_(float *trace, int *nt, float *t0, float *dt, float *sx,
                 float *sy, float *gx, float *gy, int *imute,
                 float *fovt2, float *mig, int *nxy, int *ntm, float *x,
                 float *y, float *tm, float *aperx, float *apery,
                 float *fold, float *w1, float *f0, float *df, int *nf,
                 float *ftaper, int *ifcut, int *ltaper, float *tracef,
                 float *wsave, int *nsave, float *work1, float *work2,
                 int *nfft, float *kxmax, float *kymax, float *dtau,
                 float *tau0, float *angmax, int *indxw, int *nindxw,
                 int *incdx, int *np, float *resamp, int *ires,
                 int *ntres, float *s2, float *scs, float *scg);
...
main(int argc, char **argv)
{
   ...
   float   *velos;
   float   *mig, *fold, *tm, *tmig, *tgain, *s, *l, *tugain, *trace,
   ...
   long long mlimit;
   int      tmp_mlimit;
   int      nlcore, nlpos, nlmig, *iofs, itv;
   ...
   float    o1, o2, o3, o4, o5, d1, d2, d3, d4, d5, ocdp2, oline3,
         dcdp2, dline3;
   ...
   if (!getparfloat("x1", &x1))
      err("must specify x1");
   if (!getparfloat("y1", &y1))
      err("must specify y1");
   if (!getparfloat("s1", &s1))
      err("must specify s1");
   if (!getparfloat("l1", &l1))
      err("must specify l1");
   if (!getparint("cdp1", &cdp1))
      err("must specify cdp1");
   if (!getparfloat("x2", &x2))
      err("must specify x2");
   if (!getparfloat("y2", &y2))
      err("must specify y2");
   if (!getparfloat("s2", &s2))
      err("must specify s2");
   if (!getparfloat("l2", &l2))
      err("must specify l2");
   if (!getparint("cdp2", &cdp2))
      err("must specify cdp2");
   if (!getparfloat("x3", &x3))
      err("must specify x3");
   if (!getparfloat("y3", &y3))
      err("must specify y3");
   if (!getparfloat("s3", &s3))
      err("must specify s3");
   if (!getparfloat("l3", &l3))
      err("must specify l3");
   if (!getparint("cdp3", &cdp3))
      err("must specify cdp3");
   if (!getparfloat("dds", &dds))
      err("Must specify dds!\n");
   if (!getparfloat("ddl", &ddl))
      ddl = 0.;
   if (!getparstring("velfile", &velfile))
      err("must specify velfile");

   /* optional parameters */
   if (!getparfloat("sstart", &sstart))
      sstart = s1;
   if (!getparfloat("lstart", &lstart))
      lstart = l1;
   if (!getparfloat("ds", &ds))
      ds = dds;
   if (!getparfloat("dl", &dl))
      dl = ddl;
   if (!getparint("ndl", &ndl))
      ndl = 1;
   if (ndl < 1)
      ndl = 1;
   if (!getparint("nds", &nds))
      nds = 1;
   if (nds < 1)
      nds = 1;
   ...
   if (ds < dds)
      err(" dds larger than ds ");
   if (dl < ddl)
      err(" ddl larger than dl ");
   ...
   if (!getparint("nl", &nlread))
      nlread = nlines;
   if (!getparint("ns", &nsread))
      nsread = ncdppl;
   nl = nlread;
   ns = nsread;
   if (dl == 0. && nl > 1)
      err("Must specify nonzero dl for migration");
   if (ds == 0. && ns > 1)
      err("Must specify nonzeor ds for migration");
   ...
   if (!getparint("mtrace", &mtrace))
      mtrace = 1000;
   if (!getparint("ntras", &ntras))
      ntras = 2000000000;


   if (!getparfloat("tpow", &tpow))
      tpow = -1.;
   if (!getparfloat("tpowaf", &tpowaf))
      tpowaf = 0.;
   itgain = 0;
   if (tpow != 0. || tpowaf != 0.)
      itgain = 1;
   if (!getparint("ifact", &ifact))
      ifact = 1;
   if (!getparfloat("apers", &apers))
      apers = fabs(ns * ds) / 4.;
   if (!getparfloat("aperl", &aperl))
      aperl = fabs(nl * dl) / 4.;

   if (!getparfloat("rmsmar", &rmsmar))
      rmsmar = 50.;
   ...
   if (getparfloat("send", &send)
      ...
      if (ntrend < 1)
         err("ntrend must be greater than 0 ");
      nsout = ntrend;
      nlout = 1;
   } else {
      ntrend = 0;
   ...
   if (!getparint("ntrdsk", &ntrdsk))
      ntrdsk = 0;
   ndskwt = 0;
   if (!getparint("ncpu", &ncpu))
      ncpu = 1;
   ...
   if (!getparint("traceout", &traceout))
      traceout = 1;
   if (!getparint("on2trace", &on2trace))
      on2trace = 0;
   ...
   if (getparstring("tracekey", &tracekey)
      ...
      if (!getparfloat("tr1", &tr1))
         err(" tr1 missing");
      if (!getparfloat("tr2", &tr2))
         err(" tr2 missing");
      if (!getparfloat("tr3", &tr3))
         err(" tr3 missing");
      if (!getparfloat("ln1", &ln1))
         err(" ln1 missing");
      if (!getparfloat("ln2", &ln2))
         err(" ln2 missing");
      if (!getparfloat("ln3", &ln3))
         err(" ln3 missing");
      if (!getparint("strace", &strace))
         strace = 0;
      if (!getparint("jthtrace", &jthtrace))
         jthtrace = 1;
      if (!getparint("sline", &sline))
         sline = 0;
      if (!getparint("jthline", &jthline))
         jthline = 1;
   ...
   if (ierr == 0)
      fromghdr(&gh, &scale, &dtype, &n1, &n2, &n3,
               &n4, &n5, &d1, &d2, &d3, &d4, &d5, &o1, &o2, &o3, &o4,
               &o5, &dcdp2, &dline3, &ocdp2, &oline3, &gmin, &gmax,
               &orient, &gtype);
   ...
   if (gmin == 0.) {
      if (!getparfloat("vmin", &gmin))
         err(" vmin must be specified");
   }
   ...
   if (!getparfloat("oofo", &ofomin))
      ofomin = 0.;
   if (!getparfloat("dofo", &dofo))
      dofo = 999999.;
   if (!getparfloat("obtol", &obtol))
      obtol = 0.5 * dofo;
   if (!getparint("nofo", &nofo))
      nofo = 1;
   itmp = countparname("ofo");
   if (itmp > 0 && itmp != nofo)
      err("number of ofo not match with nofo");
   ...
   if (itmp > 0) {
      ...
      for (io = 0; io < nofo; io++)
         ofo[io] = ofomin + io * dofo;
   }
   iofoin = itmp;

   if (!getparfloat("f0", &f0))
      f0 = 10.;
   if (!getparfloat("df", &df))
      df = 10.;
   if (!getparint("nf", &nf))
      nf = 7;
   if (!getparfloat("ftaper", &ftaper))
      ftaper = 5.;
   ...
   if (!getparint("tapecntl", &tapecntl))
      tapecntl = 0;
   ...
   if (!getparint("mlimit", &tmp_mlimit))
      tmp_mlimit = 256;
   mlimit = tmp_mlimit;
   mlimit *= 1024 * 1024;
   ...
   if (getparstring("hisfile", &hisfile))
      ihis = 1;
   ...
   ibacko = 0;
   if (getparstring("backupo", &backupo))
      ibacko = 1;
   if (ibacko == 1)
      isave = 1;
   ...
   if ((hffp = fopen(hisfile, "r")) != NULL) {
      ...
      do {
         bzero(buf, 81);
         if (fgets(buf, 81, hffp) == NULL)
            break;
         if (strncmp(buf, "Number of Traces Processed: ", 28) == 0) {
      ...
      if (jtrace > 0) {
         fprintf(jpfp,
                 "Number of Traces Processed for this input: %d\n",
                 jtrace);
      }
   } else {
   ...
   ltmp =
         ltmp + 2 * ntau + 2 * ntau * mtrace + 2 * nsout * nlout + ntv +
         mtrace * 4;
   ...
   ltmp =
         ltmp + (nsout * nlout * nofo + mtrace * 2 + nf * 2 +
                 nindxw * nsout * nlout) * sizeof(int);
   ...
   mig =
         ...
         (float *) emalloc(nsout * nlcore * nofo * ntau *
                           sizeof(float));
   ...
   if (ivdisk == 0)
      velos = (float *) emalloc(ntv * nsv * nlv * sizeof(float));
   ...
   if (ntrend > 0) {
      for (io = 0; io < nsout * nofo; io++)
         disend[io] = 99999999999.;
   }
   ...
   if (ntrend == 0) {
      for (il = 0; il < nlout; il++) {
         for (is = 0; is < nsout; is++) {
            s[is + il * nsout] =
                  sstart + is / nds * ds + is % nds * dds;
         }
      }
      for (il = 0; il < nlout; il++) {
         for (is = 0; is < nsout; is++) {
            l[is + il * nsout] =
                  lstart + il / ndl * dl + il % ndl * ddl;
         }
      }
   } else {
      fprintf(jpfp, " Output s and l Locations of Target Line\n");
   ...
   if (ntrpre == 0) {
      ...
      for (iy = 0; iy < nlout; iy++) {
         ...
         for (ix = 0; ix < nsout; ix++) {
            ...
            ms = s[iy * nsout + ix];
            ml = l[iy * nsout + ix];
            sl2xy(s1, l1, x1, y1, s2, l2, x2, y2, s3, l3, x3, y3, ms,
                  ml, &mx, &my);
            if (ikey == 1) {
      ...
      for (ix = 0; ix < nofo * nsout * nlout; ix++)
         off[ix] = 99999999;
      fflush(hdrfp);
   ...
   if (on2trace == 1)
      fprintf(jpfp, " total_number_of_input_trace=%d \n", ntotal);
   if (ikey == 1) {
   ...
   fhigint_(&f0, &df, &nf, &ftaper, &dtl, ifcut, ltaper, wsave, &nsave,
            &nfft);
   ...
   do {
      ...
      if (amprms == 0.) {
         ...
         if (ampmax > amprms * rmsmar) {
            fprintf(btfp,
                    " bad trace with ampmax=%g  rms=%g, at trace=%d cdp=%d cdpt=%d\n",
      ...
      if (ntrend == 0) {
         ...
         /* 
            itmp = tra.cdp - cdp1; if(cdpnum==0) { itmp = itmp/ncdppl; iiy =
            itmp; iix = tra.cdp - cdp1 - iiy*ncdppl; } else { itmp =
            itmp/nlines; iix = itmp; iiy = tra.cdp - cdp1 - iix*nlines; } */
      ...
      fprintf(stderr,
              "il=%d is=%d ml=%g ms=%g cdp=%d offset=%d io=%d\n", il,
              is, ml, ms, tra.cdp, tra.offset, io);

      if (io < 0 || io >= nofo)
         continue;

      if (ms < sinmin || ms > sinmax)
         continue;
      if (ml < linmin || ml > linmax)
         continue;
      ...
      if (itgain == 1)
         for (it = 0; it < nt; it++)
            tra.data[it] *= tgain[it];
      for (it = 0; it < nt; it++)
         tras[it + ktrace * nt] = tra.data[it];
      ...
      if (ktrace == mtrace || ktrace == ntras) {
         migs(nlout, nsout, nofo, mig, ntau,
              ...
              indxw, nindxw, incdxw,
              resamp, ires, ntres,
              ss2, scs, scg, ncpu, vr2, vi2, vq4, tau);
   ...
   if (nlcore < nlout || (ipre == 1 && ntrdsk == 0))
      efclose(imgfp);
         ...
         for (ix = 0; ix < nsout; ix++) {
            for (io = 0; io < nofo; io++) {
               ...
               if (tra.trid == 1) {
                  ...
                  ms = s[iy * nsout + ix];
                  ml = l[iy * nsout + ix];
                  sl2xy(s1, l1, x1, y1, s2, l2, x2, y2, s3, l3, x3, y3,
                        ms, ml, &mx, &my);
                  tra.sx = mx - ofs / 2;
                  tra.gx = mx + ofs / 2;
                  tra.sy = my;
               ...
               if (ikey == 1) {
                  ...
                  ml = l[iy * nsout + ix];
                  fline =
                        (ml - l1) * (ln3 - ln1) / (l3 - l1) + ln1 + 0.5;
                  ftrace =
                        (ms - s1) * (tr2 - tr1) / (s2 - s1) + tr1 + 0.5;
                  intline = fline;
                  inttrace = ftrace;
                  itov(trktype, &trkval, inttrace);
                  itov(lnktype, &lnkval, intline);
...
void     migs(int nl, int ns, int nofo, float *mig, int ntau,
              FILE * imgfp, int ktrace, int ntl, float dldt,
              float *trace, float *tras, float dtl, float tminl, int nt,
              float *ss, float *sl, float *gs, float *gl, int *imutel,
              float *fovt2, float *s, float *l, float *tm, int *iofs,
              float apers, float aperl, float *fold, float *w1,
              float *work1, float *work2, float *wsave, float *tracef,
              int *ifcut, int *ltaper, float ksmax, float klmax,
              ...
              int nfft, float dtau, float tau0, float angmax,
              int *indxw, int nindxw, int incdxw, float *resamp,
              int ires, int ntres, float *s2, float *scs, float *scg,
              int ncpu, float *vr2, float *vi2, float *vq4, float *tau)
{
   ...
   for (itr = 0; itr < ktrace; itr++) {
      ...
      pstm3d_(trace, &ntl, &tminl, &dtl, &ss[itr], &sl[itr],
              ...
              fovt2 + itr * ntau, mig + ii * ntau * nsl, &nsl, &ntau, s,
              l, tm, &apers, &aperl, fold + nsl * ii, w1, &f0, &df, &nf,
              &ftaper, ifcut, ltaper, tracef, wsave, &nsave, work1,
              work2, &nfft, &ksmax, &klmax, &dtau, &tau0, &angmax,
              indxw, &nindxw, &incdxw, &ncpu, resamp, &ires, &ntres, s2,
              scs, scg);
   }
