...
"tau0=0.                minimum output time in second			\n"
"ntau=nt                Number of output migrated samples per trace	\n" 
"dtau=dt                Output migrated time sample interval (in SECONDS)\n"
...
"                         processed (as indicated in history file) 	\n"
"                         in the last run and start to process		\n"
"                         traces right away. This option should be used \n"
"                         when more than one disk datasets are used to  \n"
"                         run ktmig.   					\n"
"                      -1=input dataset is from disk (not tape and not	\n"
"                         pipe). The program will them perform a seek	\n"
"                         to the end of the last trace processed in the	\n"
"                         previous run,	before reading and processing 	\n"
"                         the data. Used only when a single disk file	\n" 
"                         is going to be used for migration. 		\n"
...
"            ns*nds*nlcore*nofo*ntau*4 (byte)				\n" 
...
main(int argc, char **argv)
{
	...
	if(dsv==0.)  dsv = ds;
    	...
    	if (!getparfloat("tau0",&tau0)) tau0 = 0.;
	...
	fprintf(jpfp," ntrdsk=%d tpowaf=%g ncpu=%d \n",ntrdsk,tpowaf,ncpu);
