...
"vgtype=1               velocity type (1=interval 0=average)		\n"
"torz=0                 0 --- time to depth; input and velocity are time\n"
"                       1 --- depth to time; input and velocity are depth\n"
...
"    5. when torz=0, velocity grid is in time with time unit of ms;\n"
"       dt in the trace haeader will be used as time sampling interval of\n"
"       input data							\n"
"    6. when torz=1, velocity grid is in depth with depth unit of m\n"
"       or ft; dz in the trace haeader will be used as depth sampling 	\n"
...
if(iprint==1) fprintf(stderr,"line=%g trace=%g i3vgrid=%d i2vgrid=%d \n",y,x,iy+1,ix+1);
		    }
		    if(torz==0) {
			if(vgtype==1) {
				zv[0] = o1*v[0]*0.0005;
