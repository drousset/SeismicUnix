...
c	determine range of azimuth angle	
		azhn = azhmin
	    	...
	    	if(xs.eq.fxt) then
		   if(azhx.gt.PI) azhx = PI
		   if(ys.eq.fyt .and. azhx.gt.0.5*PI) azhx = 0.5*PI
		   if(ys.eq.eyt .and. azhn.lt.0.5*PI) azhn = 0.5*PI
	    	else if(xs.eq.ext) then
		   if(azhn.lt.PI) azhn = PI
		   if(ys.eq.fyt .and. azhn.lt.1.5*PI) azhn=1.5*PI
		   if(ys.eq.eyt .and. azhx.lt.1.5*PI) azhx = 1.5*PI
	    	else if(ys.eq.eyt) then
		   if(azhn.lt.0.5*PI) azhn = 0.5*PI
		   if(azhx.gt.1.5*PI) azhx = 1.5*PI
   	    	end if
...
c   avoid grid point on line direction
		tmp = (xsp(ip) - fxt)/dx
		itmp = tmp
		if(abs(tmp-itmp).lt.0.1 .and. xs.gt.100000.) xsp(ip) = xsp(ip) - 0.1
...
cccc$doacross 
ccc$par doall
cccc$doacross local(ip)
cccc$& shared(t,ov2,tt1,tt2,nxo,nyo,nzo,dxo,dyo,dzo,tmax)
	     do ip=1,np
