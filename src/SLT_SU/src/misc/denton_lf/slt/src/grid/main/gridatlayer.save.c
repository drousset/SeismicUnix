...
"               0=no will output the clipper value \n"
...
"\n"
"AUTHOR:  Zhiming Li, Unocal  2/12/2000			\n"
"\n";
...
float *vo,float dismax) {
	...
	dismaxs = dismax;
	loop_nnv:
	dismaxs2 = dismaxs * dismaxs;
	norm = 0.;
	nnv = 0;
	for(i=0;i<nv;i++) {
		/*
		tmp = sqrt( (x-xs[i])*(x-xs[i])+(y-ys[i])*(y-ys[i]) );
		*/
		tmp2 = (x-xs[i])*(x-xs[i])+(y-ys[i])*(y-ys[i]);
		/*
		if(tmp<dismaxs) {
		*/
		if(tmp2<dismaxs2) {
