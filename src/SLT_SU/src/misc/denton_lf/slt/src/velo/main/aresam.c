...
/* memory allocation */
		...
		for(i1=0;i1<n1;i1++) {
			...
			/*
			fprintf(stderr," xout=%g xin0=%g xinn=%g \n",xout[i1],xin[0],xin[nr-1]);
			*/
			for(i2=0;i2<n2-1;i2++) { 
				if(xout[i1]<xin[0]) {
					yout[i2*n1+i1] = yin[i2*nrmax];
				} else if(xout[i1]>xin[nr-1]) {
					yout[i2*n1+i1] = yin[i2*nrmax+nr-1];
				}
				fprintf(outfp," %g",yout[i2*n1+i1]);
			}
			fprintf(outfp,"\n");
		}
