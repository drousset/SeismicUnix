...
main(int argc, char **argv)
{
	...
	int maxp=250000,xlpos=2,slpos=1,tzpos=5,torz=0,vtorz=0;
	float ocdp2, dcdp2, oline3, dline3, o1, d1;
	int vgtype=1;
	float xl,sl;
	usghed usgh;
	int n1, n2, n3, i1, i2, i3, idisk, verbos;
	long long lpos;
	...
	idisk = 0;
	if(n1*n2*n3>1800000000/4) idisk = 1;
	...
	for(i1=0;i1<n1;i1++) tv[i1] = o1+i1*d1;
	for(i1=0;i1<n1;i1++) zv[i1] = o1+i1*d1;
	fseek2g(vfp,0,0);
	if(idisk==0) efread(vs,sizeof(float),n1*n2*n3,vfp);
		...
		sl = fbuf[slpos];
	    	...
	    	if(idisk==0) {
			bilint_(&n1,&n2,&n3,&ocdp2,&oline3,&dcdp2,&dline3,&xl,&sl,vs,v);
		} else {
			tmp = (xl - ocdp2)/dcdp2 + 0.5;
			i2 = tmp;
			if(i2<0) i2=0;
			if(i2>n2-1) i2=n2-1;
			tmp = (sl - oline3)/dline3 + 0.5;
			i3 = tmp;
			if(i3<0) i3=0;
			if(i3>n3-1) i3=n3-1;
			lpos = i3*n2+i2;
			...
			fseek2g(vfp,lpos,0);
			efread(v,sizeof(float),n1,vfp);
			if(verbos) fprintf(stderr," vgrid i2=%d i3=%d xl=%f sl=%f \n",i2+1,i3+1,xl,sl);
	    	}
