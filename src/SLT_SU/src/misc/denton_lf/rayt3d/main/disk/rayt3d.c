...
"                        file size) ",
"diskvxx=       disk file for velocity derivative	 		",
"diskvxy=       disk file for velocity derivative			",
"diskvxz=       disk file for velocity derivative			",
"diskvyy=       disk file for velocity derivative			",
"diskvyz=       disk file for velocity derivative			",
"diskvzz=       disk file for velocity derivative			",
"               (if the above 6 parameters are not given, ",
"               the memory is going to be used)				",
"idiskc=0       the above 6 disk files already computed (0=no 1=yes)", 
"compress=0     travel time compress ratio (0=no 5=ratio of 5) ",
" ",
"Note:	",
...
void rt3dp_(int *np,int *nzyx,int *nzyxo,int *nz,int *ny,int *nx,
  ...
  float *vv, int *idisk,
  char *filevxx, char *filevxy, char *filevxz, 
  char *filevyy, char *filevyz, char *filevzz,
  int *lenvxx, int *lenvxy, int *lenvxz,
  int *lenvyy, int *lenvyz, int *lenvzz);
...
void dv2xx_(int *nx,int *ny,int *nz,float *dx,float *dy,float *dz,float *v,
  float *vxx);
void dv2xy_(int *nx,int *ny,int *nz,float *dx,float *dy,float *dz,float *v,
  float *vxy);
void dv2xz_(int *nx,int *ny,int *nz,float *dx,float *dy,float *dz,float *v,
  float *vxz);
void dv2yy_(int *nx,int *ny,int *nz,float *dx,float *dy,float *dz,float *v,
  float *vyy);
void dv2yz_(int *nx,int *ny,int *nz,float *dx,float *dy,float *dz,float *v,
  float *vyz);
void dv2zz_(int *nx,int *ny,int *nz,float *dx,float *dy,float *dz,float *v,
  float *vzz);
...
main(int argc, char **argv)
{
	...
	char *diskvxx,*diskvxy,*diskvxz,*diskvyy,*diskvyz,*diskvzz;
	char *filevxx,*filevxy,*filevxz,*filevyy,*filevyz,*filevzz;
	int lenvxx=0,lenvxy=0,lenvxz=0,lenvyy=0,lenvyz=0,lenvzz=0;
	FILE *diskfp;
	int idisk,idiskc;
...
/*
	fprintf(stderr,"is0=%d ixs0=%d nys=%d iys0=%d \n",is0,ixs0,nys,iys0);
*/
	...
	for (is=is0;is<nxs*nys;is=is+ncpu) {
	    ...
	    for(ip=0;ip<np;ip++) {
			...
			xsp[ip] = fxs + ixs*dxs;		 
		/*
		fprintf(stderr,"ysp=%f xsp=%f ip=%d fxs=%f ixs=%d dxs=%f iys=%d dys=%f\n",
			ysp[ip],xsp[ip],ip,fxs,ixs,dxs,iys,dys);
		*/
	    }
...
/*
	fprintf(stderr,"before raytracing \n");
*/
    ...
    rt3dp_(&np,&nzyx,&nzyxo,&nz,&ny,&nx,&nzo,&nyo,&nxo,&ek,&nt,
	   ...
	   vv,&idisk,filevxx,filevxy,filevxz,filevyy,filevyz,filevzz,
	   &lenvxx,&lenvxy,&lenvxz,&lenvyy,&lenvyz,&lenvzz);
		...
		if(compress<=1.0) {
			...
			for(ixt=0;ixt<nxot;ixt++) {
				...
				for(iyt=0;iyt<nyot;iyt++) {
					...
					itemp = (ixo*nyo+iyo)*nzo;
  	    				fwrite(t+itemp+ip*nzyxo,
						sizeof(float),nzo,tfp);
				}
			}
			}
		} else {
			...
			for(ixt=0;ixt<nxot;ixt++) {
				...
				for(iyt=0;iyt<nyot;iyt++) {
					...
					if(nyot<3) iyo +=1;
					itemp = (ixo*nyo+iyo)*nzo;
      				bcopy(t+itemp+ip*nzyxo,
				      in_data+(ixt*nyot+iyt)*nzo+ip*nxyzot,
						nzo);
				}
			}
			}
...
fprintf(jpfp," travel time computed at source is=%d xs=%g ys=%g\n",
	...
	isize = nzot*nyot*nxot;
	isize = isize*nys*nxs*sizeof(float);
	...
	for(iev=0; iev<nev; ++iev){
	    ...
	    if(isres>nys*nxs) {
	    	fclose(tofp[iev]);
	    	isize = ugh.n1*ugh.n2*ugh.n3;
	    	isize = isize*ugh.n4*ugh.n5*sizeof(float);
