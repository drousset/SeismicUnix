...
int InitDsu(int argc, char **argv) /* Initialize dsu: AEM 08/16/95 */
{
  int i;
  extern DsuTask ThisDsuTask;
  extern int (*DsuGet)(FILE *, segy *);
  extern void (*DsuPut)(FILE *, segy *);
  extern int (*DsuVGet)(FILE *, segy *);
  ...
  */
  ...
  /* This Speeds up, but interfere with pvm_mcast()
  ...
  */
  ...
  /*
        Receive the TID of this process and 
        Additionally receives the father and child TID
  */

  ThisDsuTask.MasterTid =RecvInt(ThisDsuTask.branch_tid,MAX_LINK,-1,MsgBranchs);
  ThisDsuTask.MasterTid =RecvInt(&ThisDsuTask.parent_tid,1,-1,MsgParent);

  ThisDsuTask.MasterTid = RecvInt(&ThisDsuTask.srcdst[0], 1, -1, MsgWho);

  if (ThisDsuTask.srcdst[0] != 0)  /* Redirect the input */
  { 

    ThisDsuTask.srcfn = RecvStr(-1, -1);
    fprintf(stderr, " This application read input from (%s)\n", 
	  	  	ThisDsuTask.srcfn);

    if ( (ThisDsuTask.srcfd = open(ThisDsuTask.srcfn, O_RDONLY)) < 0)
      err("Error opening file <%s> (Redirecting input) \n", ThisDsuTask.srcfn);

    if (close(0) == -1)
      err("Error closing stdin \n");
    if (dup(ThisDsuTask.srcfd) != 0)
      err("Error redirecting input\n");

    DsuGet = (int (*)(FILE *, segy *) ) (fgettr);
    DsuVGet = (int (*)(FILE *, segy *) ) (fvgettr);

  }
  else 
  {
    DsuGet = (int (*)(FILE *, segy *) ) (dsufgettr);
    DsuVGet = (int (*)(FILE *, segy *) ) (dsufvgettr);
  }
      
  ThisDsuTask.MasterTid = RecvInt(&ThisDsuTask.srcdst[1], 1, -1, MsgWho);

  if (ThisDsuTask.srcdst[1] != 1) /* Redirect the output */
  {
    ThisDsuTask.dstfn = RecvStr(-1, -1);
    fprintf(stderr, " This application write output to (%s)\n", 
	  	  	ThisDsuTask.dstfn);

    /*if ((ThisDsuTask.dstfp = freopen(ThisDsuTask.dstfn, "w", stdout)) < 0) */

    if ( (ThisDsuTask.dstfd = open(ThisDsuTask.dstfn, O_WRONLY | O_CREAT | O_TRUNC, 0666)) < 0)
      err("Error opening file <%s> (Redirecting output) \n", ThisDsuTask.dstfn);

    if (close(1) == -1)
      err("Error closing stdout \n");

    if (dup(ThisDsuTask.dstfd) != 1)
      err("Error redirecting output\n");

    DsuPut = (void (*)(FILE *, segy *) ) (fputtr);
  }
  else
    DsuPut = (void (*)(FILE *, segy *) ) (dsufputtr);
      
/* DEBBUGING MESSAGES
  fprintf(stderr, "I am node <%d>, my father is (%d)\n",
                pvm_mytid(), ThisDsuTask.parent_tid);

  for (i = 0; i < MAX_LINK; i++)
      fprintf(stderr, "\tChild # %d is (%d)\n", i, ThisDsuTask.branch_tid[i]);
...
void initargs(int argc, char **argv)
{
	...
	tabled =  FALSE;
        ...
        InitDsu(argc, argv); /* AEM: 08/16/95 */
	return;
}
