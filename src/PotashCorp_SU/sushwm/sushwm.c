...
" 									",
" format=float,double,integer   format of the infile			",
" 									",
" Notes:								",
...
main(int argc, char **argv)
{
	...
	double *adfile=NULL;	/* array of "a" values from file double	*/
	int *aifile=NULL;	/* array of "a" values from file integer*/
	cwp_String format="";	/* File format float,double,integer     */
	int fform=INTEGER;
	...
	if (*infile!='\0') {
		...
		/* set from_file flag */
		from_file=cwp_true;
		getparstring("format",&format);
			if      (STREQ(format, ""       )) 	fform=FLOAT;
			else if (STREQ(format, "float"  ))	fform=FLOAT; 
			else if (STREQ(format, "double" ))	fform=DOUBLE; 
       		        else if (!STREQ(format,"integer"))
                		err("unknown file format=\"%s\", see self-doc", format);
	...
	if (!from_file) { 
		...
		switch (fform) {
			case DOUBLE:
				adfile=ealloc1double(nkeys);
			break;
			case INTEGER:
				aifile=ealloc1int(nkeys);
			break;
			default:
	...
	while (gettr(&tr)) {
		...
		if (from_file) {
			...
			switch (fform) {
				case DOUBLE:
					if (efread(adfile,DSIZE,nkeys,infp)!=0) {
						for (ikey=0; ikey<nkeys; ++ikey) {
							double a_in;
							a_in=(double) adfile[ikey];
				...
				break;
				case INTEGER:
					if (efread(aifile,ISIZE,nkeys,infp)!=0) {
						for (ikey=0; ikey<nkeys; ++ikey) {
							double a_in;
							a_in=(double) aifile[ikey];
							setval(type[ikey],&val,a_in,
							 	0,0,0,ULONG_MAX);
							puthval(&tr,index[ikey],&val);
						++count;
