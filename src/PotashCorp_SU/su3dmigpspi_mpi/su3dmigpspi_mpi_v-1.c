...
"  su3dmigpspi fs=stack fv=velocity volume                                 ",
"                                                                           ",
"									    ",
"  Required parameters:							    ",
...
int mastercode( int argc, char *argv[] )
{
	...
	float **vels;		/* velocity slice matrix */
	...
	{ register int i2,i3,iz;
	  ...
	  complex *ct;
		...
		for(iz=0;iz<nz;iz++) { 
			...
			/*bmwrite(vels,1,iz,0,1,&vslc[0][0]); */
			memcpy((void *) vels[iz],(const void *) &vslc[0][0],sizeof(float)*n2d*n3d);
		}
		free2float(vslc);
		free3float(vel);
		free1complex(ct);
	...
	{ float **swslc_r;
	  ...
	  int position = 0;
		...
		for(iz=0,z=0;iz<nz;iz++,z+=dz) {
			...
			/* Get the velocity slice at z depth */
			/* bmread(vels,1,iz,0,1,&vslc[0][0]); */
			memcpy( (void *) &vslc[0][0], (const void *) vels[iz],sizeof(float)*n2d*n3d);
			...
			if(verbose>0) fprintf(stderr," Storing results at depth z= %f\n",z);
			for(i3=0;i3<n3;i3++)
				for(i2=0;i2<n2;i2++) {
					datam[i3*n2+i2][iz] = swslc_r[i2+dmp][i3+dmp];
				}
