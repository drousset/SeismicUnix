...
char *sdoc[] = {
"							",
"SUDIF3D - generate inplane shot records for a point 	",
"          scatterer embedded in three dimensions using	",
"          the Born integral equation			",							
"							",
"	   Also can be used to create diffraction for	",
"	   3D migration test.If the zoff (zero offset)	",
"	   option is enabled.				",
"	 						",
"sudif3d [optional parameters] >stdout 			",
"							",
"Optional parameters					",
...
"	dglx=0		x-step in receiver line location",
"	dgly=0		y-step in receiver line location",
"	dglz=0		z-step in receiver line location",
"	zoff=0          zoff=1	create zero offset like	",
"			gathers. This case the shot loc-",
"			ation information is not used.	",
"			As the diffractor considered	",
"			to be the source.		",
"							",
" Example:                                              ",
...
main(int argc, char **argv)
{
	...
	float dlgx, dlgy, dlgz;		/* step in receiver line location*/
	...
	if (!getparfloat("dlgx", &dlgx))	dlgx  = 0.0;
	if (!getparfloat("dlgy", &dlgy))	dlgy  = 0.0;
	if (!getparfloat("dlgz", &dlgz))	dlgz  = 0.0;
	if (!getparint("zoff", &zoff))		zoff    = 0;
...
/*	tout = ealloc1float(nt); */
	tout = ealloc1float(nfft);
/*	for (i=0; i<nt; i++) tout[i]=i*dt; */
	for (i=0; i<nfft; i++) tout[i]=i*dt;
	...
	for (s = 0; s < nshot; ++s) {	/* loop over shots */
		...
		for (g = 0; g < nrec; ++g) {	/* loop over receivers */
			...
			if(zoff==0) {
				...
				t = rg/(c/2.0);
				spread = rg;
				/* Not sure about the k but should work for now
				*/
				amplitude = k/spread;
			}
...
/*			ints8r(1,dt,t,&amplitude,0,0,nt,tout,tr.data); */
			ints8r(1,dt,t,&amplitude,0,0,nfft,tout,rt);
...
*/
		...
		gxmin += dlgx; /* move the first receiver of the receiver line */
		gymin += dlgy;
		gzmin += dlgz;
