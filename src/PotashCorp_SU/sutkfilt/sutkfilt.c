...
/*********************** self documentation **********************/
char *sdoc[] = { " SUTKFILT -  TK domain filter			        ", 
"                       						",
"      sutkfilt < stdin > stdout [optional parameters]			",
"                       						",
"      key=ep		header keyword describing the gathers		",
"      perc=50		Cutoff percentage of the higher k values	",
...
int main( int argc, char *argv[] )
{
	...
	float perc=70;		/* cutoff percentage */
	...
	if (!getparfloat("perc", &perc)) perc = 50.0;
	...
	do {
		...
		polygonalFilter(f,amps,npoly,nfft,dx,filter);
		...
		{ register int itr,it,ifr,tsh;
			...
			for(it=0;it<nt;it++) {
				for (ifr=0; ifr<nf;++ifr) {
					 st[ifr]=
					 crmul(cmplx(fddr[ifr][it],fddi[ifr][it]),filter[ifr]);
