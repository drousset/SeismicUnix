...
"       tw=tmax        limit tpow, epow to a maximum time                         ",
...
void gain(float *data, float tpow, float epow, float gpow,
	  ...
	  register float tmin, register float dt, int nt, int nw);
...
main(int argc, char **argv)
{
	...
	int agc;        /* agc flag                                     */
        ...
        int nt,nw;         /* number of samples on trace                   */
...
)
{
		...
		tpowfac[0] = (tmin == 0.0) ? 1.0 : pow(tmin, tpow);
                for (i = 1; i < nt; ++i) {
                        if (i <= nw ) tpowfac[i] = pow(tmin + i*dt, tpow);
                        else  tpowfac[i] = pow(tmin + nw*dt, tpow);
                }
                first = cwp_false;
        } /* end first entry */
...
)
{
        ...
        if (first) {
                ...
                for (i = 0; i < nt; i++) {
                         if (i <= nw ) epowfac[i] = exp(epow * (tmin + i * dt));
                        else  epowfac[i] = exp(epow * (tmin + i * dt));
                }
                first = cwp_false;
        }
	  ...
	  register float tmin, register float dt, int nt, int nw)
{
	...
	if (tpow) {
		do_tpow(data, tpow, tmin, dt, nt, nw);
	}
	if (epow) {
		do_epow(data, epow, tmin, dt, nt,nw);
	}
