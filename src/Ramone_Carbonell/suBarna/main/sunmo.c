...
" velfile=              Velocity file name in su format (see notes)          ",
...
"									     ",
" Note: VELFILE The velocity function can be input as a SU formated file     ",
" this velfile.su. This files needs to have severaal headers definen as they ",
" provide information needed for processing. This velocity file contains     ", 
" traces which correspond to the  velocity function at a particular CDP      ",
" Therefore each velocity trace has to have the CDP number specified.        ",
"                                                                            ",
"  The headers that need to be  defined in the veloccity file are:           ",
"       ntr        Number of traces in the velocity files, correspond        ",
"                  to  the number of cdp's                                   ",
"       cdp        The CDP where thee velocity trace is located              ",
"                                                                            ",
"                                                                            ",
NULL};
...
main(int argc, char **argv)
{
	...
	int i,icnt,nv;		/* index used in loop */
        char *vfl="";	/* name of input file of header values	*/
        FILE *infp=NULL;
...
/*   Modifications by Ramon Carbonell   */

        if (getparstring("velfile",&vfl)) { 
           if((infp=efopen(vfl,"r"))==NULL) err("Cannot open VelocityFile=%s",vfl);
           if(!fgettr(infp, &vtrc)) err("can't get first velocity trace");
           ncdp=vtrc.ntr;
           nv=vtrc.ns;
	   ...
	   oa2 = ealloc2float(nt,ncdp);
           icnt=0;
           do {
              for (i=0;i<nv; i++) ovv[icnt][i]=1/(vtrc.data[i]*vtrc.data[i]);  
              icnt++;
           } while (fgettr(infp,&vtrc));
        }
